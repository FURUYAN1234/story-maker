...
`),y=((u=e.chapterRetryNotes)==null?void 0:u[a])||"",v=Hn(a,e.totalChapters,e.settings,p,d,f,n,y),$=new AbortController;e.abortController=$;try{let h="",b=e.usedModel||ue[0].value;const C=Qe(e.settings,e.headerInfo,e.totalChapters),L=Kt($.signal);b=(await me(t,b,v,({text:w,isThought:_})=>{e.active&&(_||(h+=w,xe(o,s,e.cleanText,h,{phase:`第${a}章を執筆中`,chapterNum:a})))},w=>{j(`[通信] 第${a}章: フォールバック ${w}`)},L)).usedModel;let S=0;for(;S<5;){const w=Wt(h,n,C);if(w.finished)break;S++,j(`[通信] 第${a}章: 未完了/文字数不足（本文 ${w.bodyChars.toLocaleString()} / 最低 ${w.minChars.toLocaleString()}字、raw ${w.rawChars.toLocaleString()}字、文脈メモ ${w.hasMemo?"あり":"なし"}、完結 ${w.hasFinish?"あり":"なし"}）。続きを自動リクエスト中... (${S}/5)`);const _=tr(h,n,w),z=Yt(a,n,_,C);let H="";const T=await me(t,b,z,({text:I,isThought:P})=>{if(e.active&&!P){H+=I;const R=Re(h,H,{isLast:n});xe(o,s,e.cleanText,R,{phase:`第${a}章を自動継続中`,chapterNum:a,extra:`継続 ${S}/5`})}},I=>{j(`[通信] 第${a}章: フォールバック ${I}`)},L);h=Re(h,H,{isLast:n}),b=T.usedModel}if(e.usedModel=b,!l.longNovel.active)return;let{body:A,memo:N}=Ae(stripLongNovelEmbeddedIntroMetadata(h)),k="";if(A&&A.trim().length>100){r&&(r.textContent=e.isPaused?`一時停止予約中（第${a}章後）`:`章末で一時停止（第${a}章後）`),V({phase:`第${a}章を検査中`,chapterNum:a,chapterChars:A.length,totalChars:(e.cleanText||"").length+A.length});const w=_=>{console.log(`[LN Audit Ch${a}]`,_);const z=document.getElementById("progress-log");if(z){z.textContent+=`
`+_;const H=document.getElementById("progress-content");H&&(H.scrollTop=H.scrollHeight)}};try{const _=await rt(t,A,e.settings,{onStatus:w,chapterNum:a,allContextMemos:f,recentChaptersFull:d,isLastChapter:n,fixMinorIssues:!0,failOnAuditError:!0,maxFixAttempts:2,validateFixedText:lt,sanitizeText:se});if(_.wasFixed&&(A=stripLongNovelEmbeddedIntroMetadata(_.text)),_.remainingCriticalCount>0)throw w(`[検査] 第${a}章: 重大な矛盾が${_.remainingCriticalCount}件残存 ⚠️`),new Error(`重大な設定矛盾・出力汚染が解消できなかったため、第${a}章を棄却しました（残存: ${_.remainingCriticalCount}件）`);_.wasFixed&&w(`[検査] 第${a}章: 矛盾修正が完了しました ✅`),_.issues.length>0&&(k=se(Gt(_.issues,a)))}catch(_){throw console.warn(`第${a}章の矛盾検査でエラー:`,_.message),w(`[検査] 第${a}章: 検査エラー — 保存を停止します`),new Error(`第${a}章の検査に失敗したため保存を停止しました: ${_.message||_}`)}}j(`[品質] 第${a}章: 本文保存前チェックを実行中...`),V({phase:`第${a}章の保存前チェック中`,chapterNum:a,chapterChars:A.length,totalChars:(e.cleanText||"").length+A.length}),A=stripLongNovelEmbeddedIntroMetadata(A),{body:A,memo:N}=Oe(A,N);n&&(A=ensureLongNovelFinalMarker(A));let O=Ne(A,{chapterNum:a,isLast:n,minChars:C,totalChapters:e.totalChapters});if(O.length>0&&(j(`[品質] 第${a}章: 保存前チェックで問題を検出: ${O.join(" / ")}`),A=stripLongNovelEmbeddedIntroMetadata(A),{body:A,memo:N}=Oe(A,N),n&&(A=ensureLongNovelFinalMarker(A)),O=Ne(A,{chapterNum:a,isLast:n,minChars:C,totalChapters:e.totalChapters})),O.length>0)throw new Error(`第${a}章の保存前品質ゲートで停止しました: ${O.join(" / ")}`);j(`[品質] 第${a}章: 保存前チェック通過。本文を採用します。`);const x=normalizeLongNovelChapterTitleForHeader(Vt(h),a)||deriveLongNovelChapterTitleForHeader(A,a)||buildLongNovelFallbackChapterTitle(a,e.totalChapters||a,nt(e.settings||{}),(e.headerInfo||{}).logline||"");if(e.chapters.push({title:x,body:A,contextMemo:N}),e.currentChapter=a,e.chapterRetryNotes&&delete e.chapterRetryNotes[a],e.chapterRetryCounts&&delete e.chapterRetryCounts[a],e.fullText+=`
