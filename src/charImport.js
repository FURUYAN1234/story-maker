    ...
    <div class="ci-char-card">
      ...
      <div class="ci-char-fields">
        ...
        <div class="ci-field">
          ...
          <input type="text" class="ci-input ci-sex" data-idx="${i}" value="${(c.sex || '').replace(/"/g, '&quot;')}">
        </div>
        <div class="ci-field">
...
function setupModalEvents(characters, onRegister) {
  ...
  document.getElementById('ci-btn-register').addEventListener('click', () => {
    ...
    modal.querySelectorAll('.ci-check').forEach(chk => {
      ...
      const sex = modal.querySelector(`.ci-sex[data-idx="${idx}"]`)?.value || '';
      ...
      result.push({ name, sex, role, personality, note });
    });
    onRegister(result);
    modal.remove();
  });
