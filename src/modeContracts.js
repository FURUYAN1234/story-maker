...
export const MODE_LENGTH_TARGETS = {
  ...
  '4koma_scenario': {
    ...
    target: '4800字以上',
    cleanupMax: 0,
    rule: 'Topic/Logline/Location/Outfit/Punchline/Scenario と各コマの実用情報を省略しない。コマ見出しは必ず単独行で「[1コマ目]」「[2コマ目]」「[3コマ目]」「[4コマ目]」とし、「: 起」「: 承」などの補助語を見出しに足さない。Outfitは画面化できる具体情報まで書き、指定不足をそのまま逃げ道にしない。1コマ目や4コマ目だけを散文で膨らませず、4コマすべてへ状況、絵、感情、カメラ、セリフ、演出、狙いを分配する。最新のNano Banana Pro STEP2互換として、各コマに「状況:」を置き、各コマの「セリフ:」には最低1つ、必ず「キャラ名「短いセリフ。」」形式の「」付きセリフを入れる。「無言」「セリフなし」「台詞なし」だけのコマは禁止。各コマの絵、演出、狙いはそれぞれ三文以上にし、表情、手元、背景、小物、読者に伝えたい意図を分けて書く。4コマ目も必ず「状況:」「セリフ:」「狙い:」まで書き、演出で終わらせない。検索語羅列や背景説明で膨らませず、4コマ目の狙いまで先に完走してから必要な厚みを足す。4コマ目の後に追加の小説本文を書かない。',
  },
  short_short: {
    min: 1500,
    target: '2800字以上',
...
const MODE_QUALITY_RULES = {
  ...
  '4koma_scenario': [
    ...
    '各コマは「[nコマ目]」「[EMOTION:]」「[Camera:]」「状況:」「絵:」「セリフ:」「演出:」「狙い:」をこの順の基本単位にする。4コマ目も「状況:」「セリフ:」「狙い:」まで必ず書き、演出ラベルで終了したら未完成として自分で補う。各コマの「状況:」には視覚的な状況を1〜2文で書き、「セリフ:」には最低1つ、必ず「キャラ名「短いセリフ。」」形式の吹き出し用セリフを入れる。「無言」「セリフなし」「台詞なし」だけのコマは禁止。長い検索語羅列、設定説明、同じ趣旨の反復で尺を使わず、4コマの実制作情報を最後まで完走する。指定不足をそのまま出力せず、画面化できる範囲で具体化し、4コマ目の後に別の散文段落を続けない。',
  ],
  short_short: [
