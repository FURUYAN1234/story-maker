...
【追加指示】
${e.supplement}`:"",y={"4koma":"4コマネタ","4koma_scenario":"AI 4koma シナリオ",short_short:"ショートショート",novel:"短編小説",medium:"中編小説",long:"長編小説",scenario:"脚本/台本",manga:"ストーリー漫画",essay:"エッセイ",poem:"詩・ポエム",tale:"童話/絵本",letter:"手紙/書簡体",diary:"日記/独白体",documentary:"ドキュメンタリー",radio:"ラジオドラマ"},S=e.modeCustom||y[t]||t,b={"4koma":`
...
【文体指定：ラジオドラマ】
音声のみで伝わるよう、効果音指示（SE:）・BGM指示を含め、セリフとナレーションで場面を描くこと。`}[t]||"",$=["現代","ランダム",""].includes(a)?"":`
...
・ただし、タイムスリップ等の時代錯誤がテーマ・世界観で意図されている場合はこの限りではない。`,k=pt(n,Kd),x=pt(c,Ud),I=pt(r,qd),M=pt(i,Hd),T=pt(p,Jd);let K="";k&&(K+=`
...
【語り口指定：${p}】
${T}`);let G="";t==="4koma_scenario"?G=`あなたはプロの4コマ漫画シナリオライターです。以下の設定に基づき、画像生成4コマ漫画アプリのSTEP2シナリオ入力欄に直接コピペして使える形式でシナリオを出力してください。
...
Location: [場所の詳細な描写]
Outfit: [服装を具体的に記述。人物資料がない場合も髪型・色・上着・小物などを日本語で補う]
...
[EMOTION: NORMAL]
[Camera: 俯瞰/バードアイ]
状況: [視覚的な状況を1〜2文で具体化。吹き出し以外の説明文字は描かせない]
絵: [情景・キャラクターの動作・表情を3〜5行で具体化]
セリフ: キャラ名「セリフ」 / キャラ名「セリフ」
演出: [画面内の動き、視線誘導、間の取り方]
狙い: [起として読者に何を見せるか]
...
絵: [情景・キャラクターの動作・表情を3〜5行で具体化]
セリフ: キャラ名「セリフ」 / キャラ名「セリフ」
演出: [反転、誤解、表情の崩れ、空気の変化]
狙い: [転として読者の予想をどうずらすか]

[4コマ目]
[EMOTION: CHIBI_GAG]
[Camera: 超広角/フィッシュアイ]
状況: [オチが視覚で伝わる状況を1〜2文で具体化]
絵: [オチが視覚で伝わる状況とキャラクターの反応]
セリフ: キャラ名「セリフ」 / キャラ名「セリフ」
演出: [間、沈黙、リアクション、効果音など]
狙い: [結としてどう笑い・余韻・反転を成立させるか]
...
- 各コマのセリフ欄には最低1つ、必ず キャラ名「短いセリフ。」 形式の「」付きセリフを入れること
- 「無言」「セリフなし」「台詞なし」だけのコマは禁止。全コマで吹き出し用セリフを作ること
- 1コマにつきセリフは最大2つまで
- セリフは短く、各40文字以内にすること
- 画像生成AIが描画するため、映像的・視覚的に映える動作描写を心がけること
- Scenario: の直後は必ず [1コマ目] にすること。Scenario欄の下に要約段落、あらすじ本文、全体説明を書かないこと
${v}
...
4. 全てのスコアで合格基準を達成した後、初めて '<thought>' タグを閉じ（</thought>）、その「外側」に最終的なシナリオのみを出力してください（Topic: から開始）。`:t==="long"?G=`# 厳格なシステム命令
...
\`\`\`
`:G=`あなたはプロの書き手です。以下の詳細設定に基づき、読む人の心を動かす「${S}」を執筆してください。
...
${(()=>{const B=["novel","medium","short_short","scenario","manga","documentary","radio"],Z=["essay","poem","letter","diary"];return B.includes(t)?`6. 「${c}」という結末に向かって、伏線を自然に配置すること。
...
2行目は空行。
3行目から本文を開始してください。
物語が完全に終了した際は、最後に必ず「【完】」（続く場合は「【続く】」）と記載し、文章が途切れていないことを示してください。`;let j="";e.universalAssets&&e.universalAssets.length>0&&(j=`

【入力アセット情報（インスピレーションソース）】
`,j+=`以下のユーザーから投入されたアセット情報（画像、URL、テキストなど）を、指定された「縛り（設定項目）」をすべて満たした上で、物語の要素、描写、モチーフ、設定として自然に溶け込ませて構成してください。
`,e.universalAssets.forEach((B,Z)=>{if(j+=`[アセット ${Z+1}] 型: ${B.type}
`,B.type==="image"){j+=`・画像ファイル名: ${B.name||"不明"}
`;const R=B.status==="error"?"画像解析エラーにより詳細情報なし":B.analysis||"解析中、または解析不可";j+=`・ビジュアル事前解析結果: ${R}
`}else B.type==="url"?(j+=`・リンクURL: ${B.value}
`,B.title&&B.status!=="error"&&(j+=`・リンク先タイトル: ${B.title}
`),B.content&&B.status!=="error"&&(j+=`・リンク先コンテンツ（要約/抽出テキスト）: ${B.content.slice(0,1500)}${B.content.length>1500?"...":""}
`)):B.type==="text"&&(j+=`・文書名: ${B.name||"不明"}
`,B.content&&B.status!=="error"&&(j+=`・文書内容: ${B.content.slice(0,1500)}${B.content.length>1500?"...":""}
`))}),G+=j);const W=df(e);W&&(G+=W);const se=[n,a,r,i,c,S];return e.charCount&&se.push(`${e.charCount}字`),W&&se.push("📚RAG"),e.universalAssets&&e.universalAssets.length>0&&se.push(`🖼️アセット(${e.universalAssets.length})`),{prompt:G,tags:se}}
