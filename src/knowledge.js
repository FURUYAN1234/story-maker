// ============================================================
// knowledge.js — ローカルRAG（検索拡張生成）辞書 v1.0
// ユーザーが選択した世界観・時代・テーマ等に基づき、
// 物語のディテール向上に必要な具体的知識をプロンプトに注入する。
// 外部サービス不要・完全クライアント完結型。
// ============================================================

/**
 * 知識辞書データ
 * key: ユーザー設定に含まれるキーワード（世界観・時代・テーマ等から部分一致で検索）
 * value: AIに注入する具体的な描写素材・用語・小道具・風俗・禁忌事項
 *
 * ★ 設計思想:
 *   - 1エントリ = AIが「知っているようで具体性を落としがちな領域」の補強
 *   - 小道具、衣装、食事、通貨、挨拶などの "生活レベルのディテール" を優先
 *   - AIが既に得意な抽象概念（「友情の大切さ」等）は入れない
 */
...
・小道具: 煙管（きせる）、矢立（携帯筆記具）、印籠、根付、手拭い、風呂敷`
  },

  '戦国時代': {
    tags: ['戦国', '戦国時代', '武将', '合戦', '城'],
    lore: `【戦国時代の生活知識】
...
・身分: 下剋上が横行。農民出身の武将もいる。足軽は半農半兵`
  },

  '平安時代': {
    tags: ['平安', '平安時代', '貴族', '源氏', '藤原'],
    lore: `【平安時代の生活知識】
...
・禁忌: 物忌み（ものいみ）、方違え（かたたがえ）。陰陽道に基づく禁忌を破ると凶事`
  },

  '大正時代': {
    tags: ['大正', '大正時代', '大正ロマン', '大正浪漫'],
    lore: `【大正時代の生活知識】
...
・空気: 民主主義の高揚（大正デモクラシー）と関東大震災（1923年）の影の両面`
  },
...
・禁忌: 「ちょんまげを切る」は大事件。旧幕臣と新政府の軋轢がある`
  },
...
・社会: 市民・自由民・奴隷の三層構造。パトロヌス（庇護者）とクリエンテス（被庇護者）の関係`
  },

  '中世ヨーロッパ': {
    tags: ['中世ヨーロッパ', '中世', '騎士', '封建'],
    lore: `【中世ヨーロッパの生活知識】
...
・時間: 教会の鐘が時刻の基準。朝課（マタン）〜終課（コンプリン）の祈りの時間で生活`
  },
  ...
  // ═══════════════════════════════════════
  'サイバーパンク': {
    tags: ['サイバーパンク', 'ネオン', '電脳', 'アンドロイド', 'サイバー'],
    lore: `【サイバーパンクの描写素材】
...
・比喩素材: 回路基板、電流、データストリーム、ファイアウォール、パケット、ノイズ`
  },

  'ハイファンタジー': {
    tags: ['ハイファンタジー', '異世界', '魔法', 'エルフ', 'ドワーフ', '竜', '魔王'],
    lore: `【ハイファンタジーの描写素材】
...
・比喩素材: 鍛冶の炉、蝋燭の炎、羊皮紙、印章、馬蹄、石造りの壁`
  },

  'ポストアポカリプス': {
    tags: ['ポストアポカリプス', '荒廃', '廃墟', '文明崩壊', '核の冬', 'サバイバル'],
    lore: `【ポストアポカリプスの描写素材】
...
・比喩素材: 錆、灰、砂塵、ひび割れ、静寂、残骸、苔、野生化した植物`
  },

  '宇宙': {
    tags: ['宇宙', '宇宙ステーション', '月面', '火星', 'SF', '近未来'],
    lore: `【宇宙SFの描写素材】
...
・比喩素材: 星の光、真空の静寂、金属の冷たさ、計器の点滅、ハッチのロック音`
  },
...
・和製ホラー要素: 髪の長い女、水（井戸・水溜まり）、鏡、人形、四つ角`
  },

  'ミステリー': {
    tags: ['ミステリー', '推理', '犯人', '密室', '謎', 'サスペンス', '探偵'],
    lore: `【ミステリーの構成知識】
...
・小道具: 上履き、体操服、ジャージ、名札、連絡帳、部室の鍵`
  },

  'オフィス': {
    tags: ['オフィス', '会社', 'ビジネス', '仕事', 'サラリーマン'],
    lore: `【日本のオフィスワーク・ディテール】
...
・小道具: 社員証（IDカード）、デスクの付箋、社用スマホ、ノートPC`
  },
  ...
  // ═══════════════════════════════════════
  'ソフトウェア開発': {
    tags: ['IT', 'プログラマー', 'エンジニア', 'ソフトウェア開発', 'コーディング', '開発'],
    lore: `【ソフトウェア開発現場のディテール】
...
・比喩素材: 警告（Warning）とエラー（Error）の赤文字、プログレスバー、コンパイル待ち時間`
  },

  'SIer・炎上プロジェクト': {
    tags: ['SIer', '炎上', 'デスマーチ', '客先常駐', 'システムエンジニア', 'SE'],
    lore: `【SIer・炎上プロジェクトのディテール】
...
・言葉: 「リスケ」「握る」「よしなに」「ペンディング」「人月（にんげつ）」「アサイン」`
  },

  'インフラ・ネットワーク': {
    tags: ['インフラ', 'ネットワーク', 'サーバー', 'データセンター', 'クラウド'],
    lore: `【インフラ・サーバー運用現場のディテール】
...
・言葉: 「落ちた」「pingが通らない」「再起動で復旧」「物理層の問題」「SLA」`
  },

  'AI・機械学習': {
    tags: ['AI', '機械学習', 'ディープラーニング', 'データサイエンティスト', 'AI開発'],
    lore: `【AI・機械学習の現場ディテール】
...
・比喩素材: ブラックボックス、重み（Weights）とバイアス、損失関数（Loss）のグラフ`
  },

  'ハッカー・セキュリティ': {
    tags: ['ハッカー', 'セキュリティ', 'クラッカー', 'サイバー攻撃', 'ホワイトハッカー'],
    lore: `【ハッカー・セキュリティ領域のディテール】
...
・比喩素材: ファイアウォール、暗号化の鍵、デッドドロップ、バックトレース`
  },
...
・文化的変遷: ストーカーの「ドラキュラ」→耽美系→現代ロマンス系→ダーク系へ`
  },

  'タイムトラベル': {
    tags: ['タイムトラベル', 'タイムリープ', 'タイムスリップ', '時間', 'ループ'],
    lore: `【タイムトラベルの設定素材】
...
・記憶の扱い: 時間移動者だけが記憶を保持するか、全員が変化に気づくか、が物語の性質を決める`
  },
...
/**
 * ローカルRAG検索関数
 * ユーザーの設定テキストから関連知識を検索し、プロンプトに注入する文字列を返す。
 *
 * @param {Object} settings - gatherSettings() で収集した設定オブジェクト
 * @returns {string} プロンプトに追加する知識テキスト（該当なしの場合は空文字列）
 */
export function retrieveKnowledge(settings) {
  // 検索対象テキストを結合（世界観・時代・テーマ・ジャンル・モード・補足メモ等）
  const searchText = [
    settings.worldview, settings.worldviewCustom,
    settings.era, settings.eraCustom,
    settings.theme, settings.themeCustom,
    settings.genre, settings.genreCustom,
    settings.mode, settings.modeCustom,
    settings.supplement,
    // キャラクターのメモからもキーワードを拾う
    ...(settings.characters || []).map(c => c.note || ''),
  ].filter(Boolean).join(' ');
  ...
  for (const [key, entry] of Object.entries(KNOWLEDGE_DB)) {
    ...
    // タグベースの部分一致検索
    const isMatch = entry.tags.some(tag => searchText.includes(tag));
    if (isMatch) {
      matched.push(entry.lore);
      usedKeys.add(key);
    }
  }
  ...
  // 最大3エントリまで（プロンプト肥大化防止）
  const limited = matched.slice(0, 3);
