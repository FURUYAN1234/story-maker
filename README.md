...
### v4.2.3 (2026-06-05)
- **Long-Novel QA Release Separation / 長編小説QA結果のリリース分離**: Separates the completed Gemini long-novel QA checkpoint from v4.2.2 into its own release version. The verified run completed 10 / 10 chapters with 75,502 manuscript characters, showed the completion marker and footer as `【完】` plus `Generated By AI Story Maker V4.2.2` before the version bump, and confirmed zero chapter-control or management-memo artifacts in the extracted manuscript scan. v4.2.3 keeps the same generation behavior while making the public app, footer version, release tag, and release notes align with the post-QA deploy checkpoint. / v4.2.2のまま検証結果を混ぜず、Gemini長編小説QA完走チェックポイントを独立したリリース版として分離します。検証済み実行では全10章、本文75,502字で完走し、バージョン更新前の完成表示では `【完】` と `Generated By AI Story Maker V4.2.2` が確認され、抽出本文スキャンでも章制御ラベルや管理メモ混入は0件でした。v4.2.3は生成挙動を変えず、公開アプリ、フッターバージョン、タグ、リリースノートをQA後デプロイ地点に合わせます。

### v4.2.2 (2026-06-05)
- **Provider-Accurate Model Display / APIプロバイダ別モデル表示の正確化**: OpenAI generation already routed through the OpenAI `gpt-4.1` path when an `sk-...` key was active, but the progress log still displayed the Gemini default model name during the initial connection phase. v4.2.2 now derives the initial text model and provider label from the active key/provider for standard generation, long-novel chapter 1, long-novel continuation, and post-generation editorial evaluation, so verification logs and output tags agree for both Gemini and OpenAI. / OpenAIキー使用時の生成処理自体はOpenAI `gpt-4.1` 経路に入っていましたが、初期接続ログだけがGemini既定モデル名を表示していました。v4.2.2では通常生成・長編第1章・長編次章・生成後編集評価の初期モデル/プロバイダ表示をアクティブなキー種別から決定し、Gemini/OpenAIどちらでも進捗ログと出力タグが一致するようにしました。

### v4.2.1 (2026-06-05)
- **Unified Narrative Method Stack / 物語制作メソッド共通注入**: Standard and long-novel generation now append the README narrative-engineering stack through the shared prompt contract used by both Gemini and OpenAI. The stack explicitly covers Setup-Payoff, Emotion Gap Design, Motif Recurrence, the 15-beat emotional curve, GMC+S, Show Don't Tell, Subtext, sensory balance, world-grounded metaphors, character knowledge boundaries, category-guide obedience, and multi-axis de-biasing so long-form generation does not silently bypass the app's accumulated story-making methods. The post-generation AI editorial evaluation now scores narrative-method coverage, category-guide adherence, and de-biasing in addition to the existing fact/logic, emotional arc, human-noise, constraint-fit, and commercial-prose checks. / 通常生成と長編生成の両方で、Gemini/OpenAI共通のプロンプト契約を通じて README の物語制作メソッドを明示注入するようにしました。Setup-Payoff、感情落差設計、モチーフ回帰、15ビート感情曲線、GMC+S、Show Don't Tell、サブテキスト、五感バランス、世界観準拠の比喩、キャラクター知識境界、カテゴリ文体ガイド遵守、多軸ランダム偏り抑制を明文化し、長編だけが蓄積済みメソッドを素通りしないようにしています。生成後のAI編集評価にも、物語メソッド適用、カテゴリ文体ガイド遵守、多軸ランダム偏り抑制の採点軸を追加しました。
### v4.2.0 (2026-06-05)
- **Unified Management-Block Body Guard / 管理ブロック本文混入ガード統一**: Fresh OpenAI long-novel QA on v4.1.9 completed all 10 chapters and appended the version footer, but the final manuscript scan still found two management blocks inside chapter 1: `【第1章末・構造メモ】` and `【第1章・後半補完・接続メモ】`. v4.2.0 treats chapter-end, in-chapter, continuation, supplement, structure, design, foreshadowing, roster, motif, and GMC-style memo headings as one forbidden body-contamination class. The guard removes the whole block, including following design bullets, across extraction, save-time cleanup, visible manuscript cleanup, and copy/export text; remaining management headings or design bullets are rejected by the pre-save quality gate. A fresh v4.2.0 OpenAI full-run QA passed 10 / 10 chapters with 73,830 manuscript characters, exactly one `【完】`, exactly one `Generated By AI Story Maker V4.2.0`, and zero chapter-control or management-memo artifacts in the copied manuscript. / v4.1.9 の OpenAI 長編QAでは全10章完走とバージョンフッター付与までは完了しましたが、完成稿スキャンで第1章本文内に `【第1章末・構造メモ】` と `【第1章・後半補完・接続メモ】` の2ブロックが残りました。v4.2.0 では章末、章内、接続、補完、構造、設計、伏線、ロスター、モチーフ、GMC系のメモ見出しを、同じ「本文混入禁止」カテゴリとして扱います。見出し1行だけでなく後続の設計箇条書きまでブロック単位で除去し、本文抽出、保存時クリーンアップ、可視本文クリーニング、コピー/保存対象に同じガードを適用します。さらに残存する管理見出しや設計箇条書きは保存前品質ゲートで拒否します。v4.2.0 の OpenAI 完走QAでは全10章・本文73,830字で完走し、`【完】` と `Generated By AI Story Maker V4.2.0` は各1回、コピー対象本文内の章制御ラベル・管理メモ混入はいずれも0件でした。

### v4.1.9 (2026-06-05)
- **Unified Chapter-Control Artifact Classifier / 章制御アーティファクト統一分類器**: v4.1.8 completed an OpenAI long-novel run but still allowed the standalone chapter-boundary label `（第9章 続く）` to remain between chapters 9 and 10. v4.1.9 replaces the fragment-by-fragment guard approach with one classifier for standalone chapter-control artifacts, covering Japanese continuation/completion labels, English labels such as `END CHAPTER 9`, non-numbered continuation labels, and management memo labels. The classifier is wired into tail detection, visible manuscript cleanup, tail stripping, and the pre-save quality gate so chapter-control lines are blocked before saved prose can carry them forward. / v4.1.8 の OpenAI 長編QAでは完走後、第9章と第10章の間に独立した章境界ラベル `（第9章 続く）` が残りました。v4.1.9 では断片ごとの個別ガードではなく、独立した章制御アーティファクトを判定する統一分類器へ整理し、日本語の継続/完了ラベル、`END CHAPTER 9` などの英語ラベル、章番号なしの継続ラベル、管理メモラベルをまとめて扱います。この分類器をテール検出、可視本文クリーニング、章末除去、保存前品質ゲートへ接続し、章制御行が保存本文や次章文脈へ持ち越される前に止めます。

### v4.1.8 (2026-06-05)
- **English Chapter-Control Label Guard / 英語章制御ラベルガード**: Fresh OpenAI long-novel QA on v4.1.7 was stopped at 9 / 10 chapters because the saved chapter 9 body still contained the provider-side control label `END CHAPTER 9`. v4.1.8 treats standalone English chapter labels such as `END CHAPTER 9`, `END OF CHAPTER 9`, `CHAPTER 9 END`, `CHAPTER 9 COMPLETE`, and bracketed variants as non-prose system artifacts. They are removed from visible combined output, stripped from chapter tails, and rejected by the pre-save quality gate if any remain. / v4.1.7 の OpenAI 長編QAでは 9 / 10 章まで進みましたが、第9章本文末に `END CHAPTER 9` というプロバイダ側の章制御ラベルが残ったため中断しました。v4.1.8 では `END CHAPTER 9`、`END OF CHAPTER 9`、`CHAPTER 9 END`、`CHAPTER 9 COMPLETE` および括弧付きの変種を本文ではないシステム断片として扱い、結合本文から除去し、章末から除去し、保存前ゲートで残存を拒否します。
### v4.1.7 (2026-06-05)
- **Chapter-Done Tail Label Guard / 章了ラベル断片ガード**: Fresh OpenAI long-novel QA on v4.1.6 completed all 10 chapters with 75,563 visible characters and appended `Generated By AI Story Maker V4.1.6`, but the final scan still found a chapter-completion label fragment, `＜第9章了・`, between chapters 9 and 10. v4.1.7 treats standalone chapter-done labels such as `＜第9章了・`, `（第1章、了）`, and `【第9章終了・` as non-prose tail artifacts: they are stripped from chapter tails, removed from combined visible text if encountered, and rejected by the save gate if they remain. Normal prose references such as `第9章では、...。` are explicitly preserved. / v4.1.6 の OpenAI 長編完走QAでは全10章・75,563字と `Generated By AI Story Maker V4.1.6` フッター付与までは完了しましたが、完成稿スキャンで第9章と第10章の間に章完了ラベル断片 `＜第9章了・` が残りました。v4.1.7 では `＜第9章了・`、`（第1章、了）`、`【第9章終了・` のような独立した章了ラベルを本文ではなく章末断片として扱い、章末から除去し、結合本文に残った場合も除去し、保存前ゲートで残存を拒否します。一方で `第9章では、...。` のような通常本文の章参照は明示的に保持します。

### v4.1.6 (2026-06-04)
- **Bracketed Memo and Incomplete Parenthetical Guard / 括弧付き管理メモ・未完了括弧断片ガード**: Fresh OpenAI long-novel QA on v4.1.5 completed all 10 chapters and appended `Generated By AI Story Maker V4.1.5`, but the final manuscript scan still found `【文脈維持・次章連結メモ】` inside chapter 1 and an unfinished `（続く章・展開の` tail fragment in chapter 9. v4.1.6 treats those as system-side save-quality failures, strips bracketed memo labels through the visible-body cleaner, and rejects or removes incomplete parenthetical continuation fragments before they enter saved prose. / v4.1.5 の OpenAI 長編完走QAでは全10章生成と `Generated By AI Story Maker V4.1.5` フッター付与までは完了しましたが、完成稿スキャンで第1章本文内の `【文脈維持・次章連結メモ】` と、第9章末の未完了断片 `（続く章・展開の` が残りました。v4.1.6 ではこれらをシステム側の保存品質不合格として扱い、括弧付きメモラベルを可視本文クリーナーで除去し、未完了の括弧付き続き断片を保存前に除去または拒否します。

### v4.1.5 (2026-06-04)
- **Last-Line Tail Fragment Guard / 章末最終行断片ガード**: Fresh OpenAI long-novel QA on v4.1.4 completed all 10 chapters, but the final manuscript scan still found a standalone `＜` at the end of chapter 8 and a `終章へ...` connector line at the end of chapter 9. v4.1.5 expands the save-time chapter-tail cleaner and gate to strip or reject lone bracket marks and next/final-chapter connector fragments before they enter the readable manuscript. / v4.1.4 の OpenAI 長編完走QAでは全10章生成は完了しましたが、完成稿スキャンで第8章末の単独 `＜` と第9章末の `終章へ...` 系接続文が残りました。v4.1.5 では、章末の単独括弧記号や次章・終章への接続断片を、可読本文へ入る前に保存時クリーナーと保存前ゲートで除去または拒否します。
### v4.1.4 (2026-06-04)
- **Broad Chapter-Tail Fragment Guard / 章末断片ガードの拡張**: Fresh OpenAI long-novel QA on v4.1.3 completed all 10 chapters, but the final manuscript scan still found chapter-tail fragments such as `（続く）`, `― 次章: ...`, dangling parentheses, and a label-less bullet note. v4.1.4 treats all such non-prose tail fragments as save-time cleanup/rejection targets, not acceptable manuscript text. / v4.1.3 の OpenAI 長編完走QAでは全10章の生成自体は完了しましたが、完成稿スキャンで `（続く）`、`― 次章: ...`、括弧だけの行、ラベルなし箇条書きメモが章末に残るケースを検出しました。v4.1.4 ではこれらを本文ではなく保存時に除去または拒否すべき章末断片として扱います。

### v4.1.3 (2026-06-04)
- **Label-less Outline Bullet Guard / ラベルなし箇条書き設計メモガード**: Fresh OpenAI long-novel QA on v4.1.2 completed all 10 chapters, but the final manuscript scan found several chapter endings polluted by label-less bullet outline fragments such as `・次章...`, `・布石...`, and `・導線...`. v4.1.3 treats these trailing outline bullets as formal save-quality failures, strips them from chapter tails when possible, and rejects the chapter if any remain after cleanup. / v4.1.2 の OpenAI 長編完走QAでは全10章の生成自体は完了しましたが、完成稿スキャンで `・次章...`、`・布石...`、`・導線...` のようなラベルなし箇条書き設計メモが複数章の末尾に混入していることを検出しました。v4.1.3 ではこれらを正式な保存品質不合格として扱い、可能な場合は章末から除去し、除去後も残る場合は章保存を拒否します。

### v4.1.2 (2026-06-04)
- **Dangling Chapter-Design Fragment Guard / 章末設計断片ガード強化**: OpenAI long-novel full-run QA completed all 10 chapters on v4.1.1, but the final manuscript scan found an unfinished design fragment, `【章内`, saved at the end of chapter 8. v4.1.2 extends the save-time trailing-fragment guard to strip or reject short unfinished chapter/design labels such as chapter-internal notes, chapter-end notes, scene labels, memo labels, turn/aftertaste labels, and related planning fragments before they enter the readable manuscript or carry-forward context. / v4.1.1 の OpenAI 長編完走QAでは全10章の生成自体は完了しましたが、完成稿スキャンで第8章末に `【章内` という未完了の設計断片が保存されていることを検出しました。v4.1.2 では保存時の章末断片ガードを拡張し、章内メモ、章末メモ、場面ラベル、メモラベル、ターン/余韻ラベルなどの短い未完了設計断片が、可読本文や次章文脈へ入る前に除去または保存拒否されるようにしました。
### v4.1.1 (2026-06-04)
- **OpenAI Long-Novel Paragraph Discipline / OpenAI長編の段落整形**: Long-novel prompts now explicitly forbid blank empty lines after every sentence and require single-newline paragraph breaks. Saved chapters are normalized before the pre-save quality gate, so OpenAI-style over-spaced manuscripts are corrected before they enter the readable manuscript or carry-forward context. / 長編プロンプトに「一文ごとの空行禁止」「単一改行で段落を作る」ルールを追加し、保存前チェックの前に本文を正規化します。OpenAIで出やすい空行過多の本文が、可読本文や次章文脈に入る前に整形されます。
- **Cross-Mode Output Cleanup / 他モード共通の出力整形**: Standard story output now uses the same paragraph-spacing normalizer before the version footer is appended. The long-novel quality gate also detects excessive blank lines and sends unresolved paragraph-spacing failures into the existing fail-closed retry path. / 通常生成でもフッター付与前に同じ段落整形を通します。長編保存前ゲートには空行過多・細切れ段落の検出も追加し、直せない場合は既存のfail-closed再生成へ回します。
- **Neutral API Placeholder / API入力欄の中立表示**: When no key is saved, the API input placeholder now says `Gemini または OpenAI のAPIキーを入力` instead of implying Gemini-only entry. / API未設定時の入力欄はGemini固定ではなく、OpenAIも入力できる中立表示にします。
- **Design Label Guard / 設計ラベル漏れ防止**: OpenAI long-novel drafts that leak planning labels such as chapter turns or aftertaste headings are stripped before save and rejected by the pre-save gate if residue remains. / OpenAI長編で「章のターン」「余韻」などの設計用見出しが本文へ混入した場合、保存前に除去し、残った場合は保存前ゲートで停止します。
- **Trailing Fragment Guard / 章末断片ガード**: Saved chapters now strip or reject dangling scene separators and unfinished design fragments at the very end of a chapter, such as `—`, `＊`, or an incomplete chapter label. / 章末が `—` や `＊`、途切れた章ラベルだけで終わる場合は、保存前に除去またはゲート停止します。
### v4.1.0 (2026-06-04)
...
- **Gemini Model Candidate Cleanup / Geminiモデル候補整理**: Removed the stale `gemini-3.5-flash` candidate from UI defaults and fallback lists, while preserving fallback across current Gemini 2.5/latest candidates. / 古い `gemini-3.5-flash` 候補をUI既定値とフォールバックリストから外し、Gemini 2.5系およびlatest系の候補でフォールバックする形に整理しました。
- **API Smoke Verification / APIスモーク検証**: User-entered Gemini API QA on port 5179 generated a short story successfully, appended `Generated By AI Story Maker V4.0.9`, and did not report a false invalid-key error. / ユーザー入力済みのGemini APIで実生成を行い、2,413文字のショート作品生成、`Generated By AI Story Maker V4.0.9` フッター付与、APIキー無効の誤表示なしを確認しました。
### v4.0.8 (2026-06-04)
- **Unified Version Footer / バージョン署名の統一**: Standard story outputs and completed long-novel manuscripts now use the same final footer, `Generated By AI Story Maker V4.0.8`. Long-novel final text keeps `【完】` and appends the footer after it for display, copy, and TXT save.
- **Chapter-End Noise Gate / 章末ノイズゲート**: Long-novel prompts now forbid repeated scenery-only afterglow endings, and the pre-save quality gate rejects formulaic sunset/sky/wind/light/echo-style endings unless they include concrete human friction such as a messy line, practical task, bodily discomfort, awkward silence, or unfinished obligation.
### v4.0.7 (2026-06-04)
- **Strict Configurable Axis Lock / 全設定項目ロック**: Manual entries, detailed sub-chip selections, and category-only selections are now tracked as fixed user intent. Random buttons and one-shot full random only fill unset axes or open detail slots inside the already selected parent category.
- **Random De-Biasing / ランダム偏り抑制**: Flat random idea seeds now filter unrequested digital/electronics/AI/SNS/gadget motifs unless the user explicitly selects an SF, future, cyberpunk, AI, robot, or similar axis. Prompt contracts also forbid filling unspecified gaps with digital motifs.
### v4.0.6 (2026-06-04)
