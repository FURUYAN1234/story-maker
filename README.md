...
### v4.0.9 (2026-06-04)
- **Gemini API Error Classification / Gemini APIエラー分類**: Gemini failures now separate true API-key authentication errors from model/request failures. Generic `invalid` model or request messages no longer force the misleading "API key invalid" state when diagnostics show available models.
- **Gemini Model Candidate Cleanup / Geminiモデル候補整理**: Removed the stale `gemini-3.5-flash` candidate from UI defaults and fallback lists, while preserving fallback across current Gemini 2.5/latest candidates.
- **API Smoke Verification / APIスモーク検証**: User-entered Gemini API QA on port 5179 generated a short story successfully, appended `Generated By AI Story Maker V4.0.9`, and did not report a false invalid-key error.
### v4.0.8 (2026-06-04)
