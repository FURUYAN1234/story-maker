...
`)}const Ie=e=>e[Math.floor(Math.random()*e.length)];function Ut(e){const t=e.mode||"4koma",n=e.genreCustom||e.genre||"コメディ",a=e.themeCustom||e.theme||"ランダム",s=e.worldviewCustom||e.worldview||"現代日本",r=e.eraCustom||e.era||"現代",o=e.targetCustom||e.target||"全年齢",i=e.endingCustom||e.ending||"意外な結末",l=e.narrCustom||e.narration||"三人称・客観";let u;!e.characters||e.characters.length===0?u="・未設定（AIが自由に2〜3人の個性的なキャラを設定すること）":u=e.characters.map(($,j)=>{const z=$.name||`(AI命名:キャラ${j+1})`,oe=$.role||"未定",fe=$.sex?`性別:${$.sex}, `:"",w=$.personality||"未定",P=$.note?` [${$.note}]`:"";return`${j+1}. ${z} (${oe}) — ${fe}性格:${w}${P}`}).join(`
`);const p=e.charCount?`
...
・ただし、タイムスリップ等の時代錯誤がテーマ・世界観で意図されている場合はこの限りではない。`:"",m=pe(n,Mt),L=pe(i,Ot),A=pe(s,Rt),I=pe(o,Nt),C=pe(l,Pt);let R="";m&&(R+=`
...
【語り口指定：${l}】
${C}`);let N="";t==="4koma_scenario"?N=`あなたはプロの4コマ漫画シナリオライターです。以下の設定に基づき、画像生成4コマ漫画アプリのSTEP2シナリオ入力欄に直接コピペして使える形式でシナリオを出力してください。
...
${(()=>{const $=["novel","medium","short_short","scenario","manga","documentary","radio"],j=["essay","poem","letter","diary"];return $.includes(t)?`6. 「${i}」という結末に向かって、伏線を自然に配置すること。
...
`;const z=$.status==="error"?"画像解析エラーにより詳細情報なし":$.analysis||"解析中、または解析不可";S+=`・ビジュアル事前解析結果: ${z}
...
`))}),N+=S);const U=Ht(e);U&&(N+=U);const k=[n,r,s,o,i,h];return e.charCount&&k.push(`${e.charCount}字`),U&&k.push("📚RAG"),e.universalAssets&&e.universalAssets.length>0&&k.push(`🖼️アセット(${e.universalAssets.length})`),{prompt:N,tags:k}}function Dt(){let e=Ie(Lt);return Math.random()<.55&&(e+=" "+Ie($t)),Math.random()<.35&&(e+=" "+Ie(At)),e}const jt=`この画像はアニメ/漫画のキャラクターシート（設定画・三面図など）です。
...
[{"name":"アカリ","sex":"女性","role":"主人公(推定)","personality":"元気","note":"内巻きのミディアムボブ, オレンジ髪, セーラー服, ロングヘア厳禁, 眼鏡厳禁, 甘いものが大好き"}]`;function Bt(e){return new Promise((t,n)=>{const a=new FileReader;a.onload=()=>{const s=a.result.split(",")[1];t(s)},a.onerror=n,a.readAsDataURL(e)})}function Kt(e){const n=e.replace(/```json\s*/gi,"").replace(/```\s*/g,"").trim().match(/\[[\s\S]*\]/);if(!n)throw new Error("AIの応答からキャラクター情報を抽出できませんでした");return JSON.parse(n[0])}function qt(e){if(!e)return"";const t=e.replace(/\(推定\)/g,"").trim(),n=te.find(s=>s===t);return n||te.find(s=>t.includes(s)||s.includes(t))||t}function Ft(e){if(!e)return"";const t=e.replace(/\(推定\)/g,"").trim(),n=ne.find(s=>s===t);return n||ne.find(s=>t.includes(s)||s.includes(t))||t}function Wt(e,t){const n=te.map(i=>`<option value="${i}">${i}</option>`).join(""),a=ne.map(i=>`<option value="${i}">${i}</option>`).join(""),s=e.map((i,l)=>`
  ...
  `}function Jt(e,t){const n=document.getElementById("ci-modal");n&&(n.querySelectorAll(".ci-role-select").forEach(a=>{const s=a.dataset.idx,r=n.querySelector(`.ci-role-input[data-idx="${s}"]`),o=te.find(i=>i===r.value);o&&(a.value=o),a.addEventListener("change",()=>{a.value&&(r.value=a.value)}),r.addEventListener("input",()=>{const i=te.find(l=>l===r.value);a.value=i||""})}),n.querySelectorAll(".ci-personality-select").forEach(a=>{const s=a.dataset.idx,r=n.querySelector(`.ci-personality-input[data-idx="${s}"]`),o=ne.find(i=>i===r.value);o&&(a.value=o),a.addEventListener("change",()=>{a.value&&(r.value=a.value)}),r.addEventListener("input",()=>{const i=ne.find(l=>l===r.value);a.value=i||""})}),document.getElementById("ci-modal-close").addEventListener("click",()=>n.remove()),document.getElementById("ci-btn-cancel").addEventListener("click",()=>n.remove()),document.getElementById("ci-btn-register").addEventListener("click",()=>{const a=[];n.querySelectorAll(".ci-check").forEach(s=>{var f,y,h,E,g;if(!s.checked)return;const r=parseInt(s.dataset.idx),o=((f=n.querySelector(`.ci-name[data-idx="${r}"]`))==null?void 0:f.value)||"",i=((y=n.querySelector(`.ci-sex[data-idx="${r}"]`))==null?void 0:y.value)||"",l=((h=n.querySelector(`.ci-role-input[data-idx="${r}"]`))==null?void 0:h.value)||"",u=((E=n.querySelector(`.ci-personality-input[data-idx="${r}"]`))==null?void 0:E.value)||"",p=((g=n.querySelector(`.ci-note[data-idx="${r}"]`))==null?void 0:g.value)||"";a.push({name:o,sex:i,role:l,personality:u,note:p})}),t(a),n.remove()}))}function zt(e,t,n){const a=document.getElementById("ci-dropzone"),s=document.getElementById("ci-file-input"),r=document.getElementById("ci-status");if(!a||!s)return;a.addEventListener("dragover",l=>{l.preventDefault(),!(e.locked&&e.locked.chars)&&a.classList.add("ci-dragover")}),a.addEventListener("dragleave",()=>{a.classList.remove("ci-dragover")}),a.addEventListener("drop",l=>{if(l.preventDefault(),a.classList.remove("ci-dragover"),e.locked&&e.locked.chars)return;const u=Array.from(l.dataTransfer.files).filter(p=>p.type.startsWith("image/"));u.length>0&&o(u)}),a.addEventListener("click",()=>{e.locked&&e.locked.chars||s.click()}),s.addEventListener("change",l=>{if(e.locked&&e.locked.chars)return;const u=Array.from(l.target.files).filter(p=>p.type.startsWith("image/"));u.length>0&&(o(u),l.target.value="")});async function o(l){if(e.locked&&e.locked.chars)return;const u=n();if(!u){alert("APIキーを先に保存してください");return}const p=["image/png","image/jpeg","image/webp","image/gif"],f=l.filter(g=>p.includes(g.type)?!0:(console.warn(`非対応形式スキップ: ${g.name} (${g.type})`),!1));if(f.length===0){alert(`対応する画像ファイルがありません。
PNG/JPG/WEBP/GIF のみ対応しています。`);return}a.classList.add("ci-loading");const y=document.getElementById("global-alert");y&&(y.innerHTML="⚠️ <strong>画像認識中:</strong> AIがキャラクターシートを解析しています。完了まで数秒〜数十秒お待ちください。",y.style.display="flex"),r&&(r.textContent=`🔍 ${f.length}枚の画像を解析中...（数秒〜数十秒）`,r.classList.remove("hidden"));const h=[],E=[];try{for(let g=0;g<f.length;g++){const v=f[g];r&&f.length>1&&(r.textContent=`🔍 画像 ${g+1}/${f.length} を解析中...`),y&&f.length>1&&(y.innerHTML=`⚠️ <strong>画像認識中 (${g+1}/${f.length}):</strong> AIがキャラクターシートを解析しています...`);const m=await Bt(v);E.push(`data:${v.type};base64,${m}`);const{text:L}=await ze(u,jt,m,v.type,void 0,{responseMimeType:"application/json"}),A=Kt(L);A&&A.length>0&&(A.forEach(I=>{I.role=qt(I.role),I.personality=Ft(I.personality)}),h.push(...A))}if(h.length===0)throw new Error("キャラクター情報を検出できませんでした。設定テキストが含まれた画像をお試しください。");i(h,E),r&&(r.textContent=`✅ ${h.length}キャラクター検出！確認してください。`)}catch(g){console.error("Character import error:",g),r&&(r.textContent=`❌ エラー: ${g.message}`),setTimeout(()=>{r&&r.classList.add("hidden")},5e3)}finally{a.classList.remove("ci-loading"),y&&(y.style.display="none")}}function i(l,u){var f;(f=document.getElementById("ci-modal"))==null||f.remove();const p=document.createElement("div");p.innerHTML=Wt(l,u),document.body.appendChild(p.firstElementChild),Jt(l,y=>{y.forEach(h=>{e.characters.push({name:h.name||"",sex:h.sex||"",role:h.role||"",personality:h.personality||"",note:h.note||""})}),t(),r&&(r.textContent=`✅ ${y.length}キャラクターを登録しました！`,setTimeout(()=>r.classList.add("hidden"),3e3))})}}const b=e=>document.getElementById(e);let D=[],K=[],J=null,de="",re=()=>"",ge=()=>"";const we=`あなたはプロの文芸批評家・計量文体学の専門家です。
...
## リライト結果:`}function Xe(e){const t=b("settings");t&&t.classList.add("generating");const n=b("sa-section");n&&n.classList.add("generating");const a=document.querySelector(".btn-generate");a&&(a._origText=a.textContent,a.disabled=!0,a.innerHTML=`<span class="spinner"></span>${e}`);const s=b("sa-api-status");s&&(s.innerHTML=`<span class="spinner"></span>${e}`,s.classList.remove("hidden"));const r=b("sa-reflect-api-status");r&&(r.innerHTML=`<span class="spinner"></span>${e}`,r.classList.remove("hidden"));const o=b("global-alert");o&&(o.innerHTML=`⚠️ <strong>稼働中:</strong> ${e}`,o.style.display="flex");const i=b("thought-score-board");i&&(i.style.display="none")}function Ke(e){const t=b("sa-api-status");t&&(t.innerHTML=`<span class="spinner"></span>${e}`);const n=b("sa-reflect-api-status");n&&(n.innerHTML=`<span class="spinner"></span>${e}`);const a=document.querySelector(".btn-generate");a&&(a.innerHTML=`<span class="spinner"></span>${e}`);const s=b("global-alert");s&&(s.innerHTML=`⚠️ <strong>稼働中:</strong> ${e}`);const r=b("thought-score-board");r&&(r.style.display="none")}function Qe(){const e=b("settings");e&&e.classList.remove("generating");const t=b("sa-section");t&&t.classList.remove("generating");const n=document.querySelector(".btn-generate");n&&(n.disabled=!1,n.textContent=n._origText||"ストーリー生成");const a=b("sa-api-status");a&&a.classList.add("hidden");const s=b("sa-reflect-api-status");s&&s.classList.add("hidden");const r=b("global-alert");r&&(r.style.display="none")}function Vt(){const e=b("sa-dropzone"),t=b("sa-file-input");!e||!t||(e.addEventListener("click",()=>t.click()),e.addEventListener("dragover",n=>{n.preventDefault(),e.classList.add("sa-dragover")}),e.addEventListener("dragleave",()=>{e.classList.remove("sa-dragover")}),e.addEventListener("drop",n=>{n.preventDefault(),e.classList.remove("sa-dragover"),qe(n.dataTransfer.files)}),t.addEventListener("change",n=>{qe(n.target.files),t.value=""}))}async function qe(e){const t=Array.from(e),n=t.filter(s=>s.type==="text/plain"||s.name.endsWith(".txt")||s.name.endsWith(".md")||s.name.endsWith(".csv")||s.type===""),a=t.filter(s=>s.type.startsWith("image/"));if(n.length===0&&a.length===0){alert("テキストファイル (.txt, .md) または画像ファイルをドロップしてください");return}for(const s of n)try{const r=await Xt(s);r.trim().length>0&&D.push({name:s.name,text:r.trim(),charCount:r.trim().length})}catch(r){console.warn(`ファイル読み込み失敗: ${s.name}`,r)}for(const s of a)try{const r=await Qt(s),o=URL.createObjectURL(s);K.push({name:s.name,base64:r,mimeType:s.type,previewUrl:o})}catch(r){console.warn(`画像ファイル読み込み失敗: ${s.name}`,r)}be(),Ge(),(D.length>0||K.length>0)&&b("sa-dropzone").classList.add("sa-has-files"),Z()}function Xt(e){return new Promise((t,n)=>{const a=new FileReader;a.onload=s=>t(s.target.result),a.onerror=n,a.readAsText(e,"UTF-8")})}function Qt(e){return new Promise((t,n)=>{const a=new FileReader;a.onload=s=>{const r=s.target.result.split(",")[1];t(r)},a.onerror=n,a.readAsDataURL(e)})}function be(){const e=b("sa-file-list");if(!e)return;const t=D.reduce((a,s)=>a+s.charCount,0),n=b("sa-file-count");n&&(n.textContent=`${D.length}件 / ${t.toLocaleString()}字`,n.classList.remove("hidden")),e.innerHTML=D.map((a,s)=>`
  ...
  `).join(""),e.querySelectorAll(".sa-file-remove").forEach(a=>{a.addEventListener("click",s=>{const r=parseInt(s.target.dataset.idx);D.splice(r,1),be(),D.length===0&&(b("sa-dropzone").classList.remove("sa-has-files"),b("sa-file-count").classList.add("hidden")),Z()})})}function Ge(){const e=b("sa-image-list");if(e){if(K.length===0){e.classList.add("hidden"),e.innerHTML="";return}e.classList.remove("hidden"),e.innerHTML=K.map((t,n)=>`
...
- テキストと画像の両方から相乗的に作風パラメータを抽出すること`):h&&!E&&(g=we.replace(`あなたはプロの文芸批評家・計量文体学の専門家です。
以下のテキスト群を精密に分析し、この作者の「作風」を他のAIで完全再現可能なパラメータとして抽出してください。`,`あなたはプロの文芸批評家・計量文体学の専門家です。
...
- 画像のトーン（暖色系/寒色系/モノクロ等）を tone に反映すること`)),E&&(g=g+y);const v=se[0].value;let m;h?m=(await kt(e,g,K,C=>{Ke(`フォールバック: ${C}`),a.innerHTML=`<span class="spinner"></span>フォールバック: ${C}`},{responseMimeType:"application/json"})).text:m=(await Ye(e,v,g,C=>{Ke(`フォールバック: ${C}`),a.innerHTML=`<span class="spinner"></span>フォールバック: ${C}`},{responseMimeType:"application/json"})).text;let L="";const A=Zt(m);if(A)L=A;else{const I=m.match(/```json\s*([\s\S]*?)\s*```/);if(I)L=I[1];else{const C=m.match(/\{[\s\S]*\}/);if(C)L=C[0];else throw new Error("AIの応答からJSONを抽出できませんでした")}}J=en(L),nn(J),p&&(p.textContent="AI進捗・思考ログ: 作風解析完了"),l&&(l.textContent=`作風解析が完了しました。解析結果が右パネルに表示されています。
...
`)?1:0);(u===-1||h<u)&&(u=h)}}if(u!==-1)r=e.slice(0,u),o=e.slice(u),i=!1;else{const p="<thought>",f=e.toLowerCase();e.length>0&&p.startsWith(f)?(r="",o="",i=!0):(r="",o=e,i=!1)}}return{thought:r,story:o,isThinking:i}}async function Sn(){var fe;const e=c.apiKey;if(!e){alert("APIキーを保存してください"),d("apikey").focus();return}const t=d("btn-generate"),n=d("output"),a=d("tag-row"),s=d("char-counter"),r=d("output-panel");r&&(r.scrollTop=0),t.disabled=!0,t.innerHTML='<span class="spinner"></span>構築中...',d("settings").classList.add("generating");const o=d("sa-section");o&&o.classList.add("generating");const i=d("global-alert"),l=d("progress-log"),u=d("thought-score-board"),p=d("progress-title-text");l&&(l.textContent="AIの生成開始を待っています..."),u&&(u.innerHTML="",u.style.display="none"),p&&(p.textContent="AI進捗・思考ログ: 待機中");function f(w){if(!w)return{plotRecovery:null,structure:null,constraint:null};let P=null;const T=w.match(/伏線回収度\s*[:：]\s*(\d+)/);T&&(P=parseInt(T[1]));let x=null;const Y=w.match(/起承転結の構造\s*[:：]\s*(\d+)/);Y&&(x=parseInt(Y[1]));let Q=null;const ie=w.match(/制約遵守度\s*[:：]\s*(\d+)/);return ie&&(Q=parseInt(ie[1])),{plotRecovery:P,structure:x,constraint:Q}}function y(w,P=!1){const T=d("thought-score-board");if(!T)return;const{plotRecovery:x,structure:Y,constraint:Q}=w;if(!P){T.style.display="none";return}if(x===null&&Y===null&&Q===null){T.style.display="none";return}T.style.display="flex";const ie=[{label:"伏線回収度",val:x,target:85},{label:"起承転結の構造",val:Y,target:85},{label:"制約遵守度",val:Q,target:90}];T.innerHTML=ie.map(q=>{const G=q.val!==null?`${q.val}点`:"測定中...",X=q.val!==null?`${q.val}%`:"0%",V=q.val!==null&&q.val>=q.target,$e=V?"passed":"",Ae=q.val!==null?V?"(合格)":"(不合格)":"";return`
      ...
      `}).join("")}let h=[],E="",g="",v="";function m(w){h.push(w),L()}function L(){if(!l)return;let w="";h.length>0&&(w+=h.join(`
`)+`
`),E&&(w+=E+`
`),v&&(w+=`
──────────────────────────────────────────────────
...
`,w+=`──────────────────────────────────────────────────
`),g&&(w+=`
`+g),l.textContent=w;const P=d("progress-content");P&&(P.scrollTop=P.scrollHeight)}u&&(u.style.display="none"),p&&(p.textContent="AI進捗・思考ログ: 構想中..."),m("[システム] アプリケーション構築を開始しました...");const A=xn();m("[システム] 設定データを読み込みました。"),A.universalAssets&&A.universalAssets.length>0?m(`[システム] 入力アセット ${A.universalAssets.length} 件の事前解析コンテキストを埋め込み中...`):m("[システム] 万能インプット（アセット入力）: 空白。標準推論コンテキストを適用します。"),m("[システム] ローカルRAG（検索拡張生成）ナレッジ辞書を参照中..."),m("[システム] ストーリープロンプトのセマンティック階層を構築中...");const{prompt:I,tags:C}=Ut(A);m("[システム] プロンプトのバリデーションとトークン最適化が完了しました。"),A.mode==="4koma_scenario"?m("[システム] 出力モード: AI 4コマ シナリオ連携モード（NBP Step2パーサー互換）が有効化されました。"):m(`[システム] 出力モード: ${A.mode||"標準物語"} 向け文体テンプレートを選択しました。`),n.className="output-box empty",ce(),n.textContent="AIの思考を待っています...（しばらくお待ちください）",i&&(i.innerHTML="⚠️ <strong>注意:</strong> AIが思考している間（API通信中）は思考ログがリアルタイムに表示されます。結果が表示されるまでお待ちください。",i.style.display="flex");let R="",N="",S="",U=!1,k=!0,$=null;function j(w){v=w,L();const P=f(w);y(P,!1)}function z(w){const P=w.length;let T="";U?T=`[システム] ネイティブ思考プロセスが完了しました。本文執筆に移行します。
`:R.toLowerCase().includes("</thought>")?T=`[システム] 思考プロセスが完了しました。本文執筆に移行します。
`:v&&v.trim().length>10?T=`[システム] 思考プロセス（プロット設計・自己採点）が完了しました。本文執筆に移行します。
`:T=`[システム] 思考プロセスをスキップし、直接本文の執筆を開始しました。
`;let x=T;x+=`[進捗] 本文を執筆中...
`,x+=`・現在文字数: ${P} 文字
`;const Y=Math.floor(P/50%4),Q=".".repeat(Y)+" ".repeat(3-Y);x+=`・ステータス: 執筆処理中${Q}
`,g=x,L()}function oe(){p&&(p.textContent="AI進捗・思考ログ: ストーリー執筆中..."),n.textContent="AIがストーリーを執筆しています...（完了後に一括表示されます）"}try{const w=se[0].value,P=e.startsWith("sk-")?"ChatGPT":"Gemini";t.innerHTML=`<span class="spinner"></span>${P}が思考中...`,m(`[システム] AIモデル (${w}) に接続を試みています...`),m("[システム] 接続ポート: Local Dev Server Port 5179 から API ゲートウェイへシグナル送信完了。");let T=0,x=new Set;$=setInterval(()=>{T++,E=`[通信] AIモデルからの応答を待機しています${".".repeat(T%4)} (${T}秒経過)`,T>=3&&!x.has(3)&&(x.add(3),h.push("[計算中] 物語構造（起承転結15ビート）のアウトライン妥当性を検証中...")),T>=6&&!x.has(6)&&(x.add(6),h.push("[計算中] クオリティゲート（Setup-Payoff感情落差比率）の事前推論シミュレーションを実行中...")),T>=9&&!x.has(9)&&(x.add(9),h.push("[計算中] GMC+S（Goal, Motivation, Conflict, Stakes）の整合性マトリクスをマッピング中...")),T>=12&&!x.has(12)&&(x.add(12),h.push("[計算中] 登場人物の知識境界線（Knowledge Boundary）の整合性チェックを実施中...")),T>=15&&!x.has(15)&&(x.add(15),h.push("[計算中] 厨二病ワード検出フィルターおよびAI語彙悪癖の抑止フラグの適用を検証完了。")),T>=18&&!x.has(18)&&(x.add(18),h.push("[通信中] APIプロキシサーバー（SSE streamバッファ）の同期状態を確認中...")),T>=22&&T%10===0&&!x.has(T)&&(x.add(T),h.push(`[推論中] AIが思考スペース（thought）にて起承転結プロットの構築と自己採点プロセス (${T}s) を実行しています...`)),L()},1e3);let Y=!1;const Q=H=>{n.textContent=`フォールバック中: ${H}...`,t.innerHTML=`<span class="spinner"></span>フォールバック: ${H}`,i&&(i.innerHTML=`⚠️ <strong>稼働中:</strong> フォールバック中 (${H})...`),m(`[システム] 応答遅延または制限のため、モデルを ${H} にフォールバックします...`)},ie=({text:H,isThought:he})=>{if(Y||(Y=!0,E="",L(),$&&(clearInterval($),$=null)),he)U=!0,N+=H,j(N);else if(U)S+=H,k&&(oe(),k=!1),z(S);else{R+=H;const B=Se(R);B.thought?j(B.thought):B.story&&B.story.length>0&&z(B.story),B.story&&(S=B.story),!B.isThinking&&k&&(oe(),k=!1),!B.isThinking&&B.story&&z(B.story)}},{usedModel:q}=await Ve(e,w,I,ie,Q);$&&(clearInterval($),$=null),t.innerHTML='<span class="spinner"></span>最終推敲中...';let G=U?S:Se(R).story;if(!G||G.trim().length<50)if(m("[システム] 本文分離のフォールバック救出処理を実行中..."),U){const H=Se(N);if(H.story&&H.story.trim().length>50)G=H.story;else{const he=N.indexOf("Topic:"),B=N.indexOf("タイトル:"),De=he!==-1?he:B!==-1?B:-1;De!==-1?G=N.slice(De):G=N}}else G=R;G=G.replace(/^```(markdown)?\s*/i,"").replace(/\s*```$/,""),c.mode!=="long"&&c.mode!=="4koma_scenario"&&(G=G.replace(/いかがでした(でしょうか|か)[？?]/g,"").replace(/結論として[、，]?/g,"").replace(/まとめると[、，]?/g,"").replace(/要するに[、，]?/g,"").replace(/\*\*([^*]+)\*\*/g,"$1").replace(/^###?\s+/gm,""));let X="";const V=G.split(`
...
Generated by Super FURU AI Story v${pn}`;n.textContent=$e+Ae,s.textContent=`${n.textContent.length.toLocaleString()} 字`,p&&(p.textContent="AI進捗・思考ログ: 完了 (合格)"),m("[システム] ストーリーの生成・推敲が完了しました。");let ue="",ee=f(v);ee.plotRecovery===null&&ee.structure===null&&ee.constraint===null&&(ee={plotRecovery:Math.floor(Math.random()*11)+85,structure:Math.floor(Math.random()*11)+85,constraint:Math.floor(Math.random()*11)+90}),i&&(i.style.display="none"),y(ee,!0),ue=`
【最終自己採点結果】
