...
[各モデルのエラー詳細]
${o.join(`
`)}`,new Error(b)}async function Qe(e,t,a,n,s={}){if(e.trim().startsWith("sk-"))return Tt(e.trim(),a,n,s);const r=["gemini-3.5-flash","gemini-2.5-flash","gemini-2.5-pro","gemini-flash-latest","gemini-pro-latest"],i=new Set([t,...r,...le.map(y=>y.value)]),o=Array.from(i),d=[];let p=!1,u=!1,h=!1;for(const y of o)try{return y!==t&&n&&n(y),{text:await wt(e,y,a,s),usedModel:y}}catch(f){const v=f.message||"";console.warn(`Model ${y} failed:`,v),d.push(`${y}: ${v}`);const m=v.toLowerCase();(m.includes("safety")||m.includes("prohibited")||m.includes("block"))&&(p=!0),(m.includes("quota")||m.includes("429")||m.includes("limit"))&&(u=!0),(m.includes("api key")||m.includes("403")||m.includes("invalid"))&&(h=!0);continue}console.log("All models failed. Running diagnosis...");const b=await _e(e);console.error("DIAGNOSIS RESULT:",b);let g=`全モデル接続失敗: ${b}
...
[各モデルのエラー詳細]
${d.join(`
`)}`,new Error(g)}const ye=["gpt-4.1","gpt-4.1-mini","gpt-4.1-nano","gpt-4o"];async function Tt(e,t,a,n={}){var s,r,i,o,d,p;for(const u of ye)try{u!==ye[0]&&a&&a(u);const h=await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${e}`},body:JSON.stringify({model:u,messages:[{role:"user",content:t}],temperature:1,max_tokens:8192,response_format:n.responseMimeType==="application/json"?{type:"json_object"}:void 0})});if(!h.ok){const y=await h.json().catch(()=>({}));throw new Error(`OpenAI HTTP ${h.status} - ${((s=y.error)==null?void 0:s.message)||h.statusText}`)}const b=await h.json(),g=((o=(i=(r=b.choices)==null?void 0:r[0])==null?void 0:i.message)==null?void 0:o.content)||"";if(!g)throw new Error(`Empty response (FinishReason: ${((p=(d=b.choices)==null?void 0:d[0])==null?void 0:p.finish_reason)||"UNKNOWN"})`);return{text:g,usedModel:u}}catch(h){console.warn(`Model ${u} failed:`,h.message);continue}throw new Error("全モデル接続失敗: OpenAI API Keyが無効か、使用回数の上限（Quota Exceeded）に達しています。")}const Be=["gpt-4.1","gpt-4o","gpt-4.1-mini"];async function St(e,t,a,n,s,r={}){var o,d,p,u,h,b;const i=`data:${n};base64,${a}`;for(const g of Be)try{Be[0];const y=await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${e}`},body:JSON.stringify({model:g,messages:[{role:"user",content:[{type:"text",text:t},{type:"image_url",image_url:{url:i,detail:"high"}}]}],temperature:.3,max_tokens:8192,response_format:r.responseMimeType==="application/json"?{type:"json_object"}:void 0})});if(!y.ok){const m=await y.json().catch(()=>({}));throw new Error(`OpenAI HTTP ${y.status} - ${((o=m.error)==null?void 0:o.message)||y.statusText}`)}const f=await y.json(),v=((u=(p=(d=f.choices)==null?void 0:d[0])==null?void 0:p.message)==null?void 0:u.content)||"";if(!v)throw new Error(`Empty response (FinishReason: ${((b=(h=f.choices)==null?void 0:h[0])==null?void 0:b.finish_reason)||"UNKNOWN"})`);return{text:v,usedModel:g}}catch(y){console.warn(`Vision Model ${g} failed:`,y.message);continue}throw new Error("全モデルでの画像認識に失敗: OpenAI API Keyが無効か、使用回数の上限に達しています。")}async function kt(e,t,a,n,s={}){var h,b,g,y;const r=`https://generativelanguage.googleapis.com/v1beta/models/${t}:generateContent?key=${e}`,i=[{text:a}];n.forEach(f=>{i.push({inlineData:{mimeType:f.mimeType,data:f.base64}})});const o={temperature:s.temperature!==void 0?s.temperature:.4};s.responseMimeType&&(o.responseMimeType=s.responseMimeType);const d=s.timeoutMs||6e4,p=new AbortController,u=setTimeout(()=>p.abort(),d);try{const f=await fetch(r,{method:"POST",headers:{"Content-Type":"application/json"},signal:p.signal,body:JSON.stringify({contents:[{parts:i}],generationConfig:o,safetySettings:[{category:"HARM_CATEGORY_HARASSMENT",threshold:"BLOCK_NONE"},{category:"HARM_CATEGORY_HATE_SPEECH",threshold:"BLOCK_NONE"},{category:"HARM_CATEGORY_SEXUALLY_EXPLICIT",threshold:"BLOCK_NONE"},{category:"HARM_CATEGORY_DANGEROUS_CONTENT",threshold:"BLOCK_NONE"}]})});if(clearTimeout(u),!f.ok){const m=await f.text();let E=`Gemini HTTP ${f.status}`;try{const L=JSON.parse(m);L.error&&L.error.message&&(E+=` — ${L.error.message}`)}catch{E+=` — ${m.slice(0,300)}`}throw new Error(E)}const v=await f.json();if((h=v.promptFeedback)!=null&&h.blockReason)throw new Error(`Blocked by Safety Filter: ${v.promptFeedback.blockReason}`);if((y=(g=(b=v.candidates)==null?void 0:b[0])==null?void 0:g.content)!=null&&y.parts){const m=v.candidates[0].content.parts.map(E=>E.text||"").join("");if(!m){const E=v.candidates[0].finishReason||"UNKNOWN";throw new Error(`Empty response (FinishReason: ${E}).`)}return m}throw v.error?new Error(`Gemini API Error: ${v.error.message}`):new Error("No response candidates (Unknown Model Refusal)")}catch(f){throw f.name==="AbortError"?new Error(`Timeout: ${t} multimodal (${d/1e3}s)`):f}finally{clearTimeout(u)}}async function Ct(e,t,a,n,s={}){var i,o,d,p,u,h;const r=["gpt-4.1","gpt-4o","gpt-4.1-mini"];for(const b of r)try{b!==r[0]&&n&&n(b);const g=[{type:"text",text:t}];a.forEach(m=>{g.push({type:"image_url",image_url:{url:`data:${m.mimeType};base64,${m.base64}`,detail:"high"}})});const y=await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${e}`},body:JSON.stringify({model:b,messages:[{role:"user",content:g}],temperature:.4,max_tokens:8192,response_format:s.responseMimeType==="application/json"?{type:"json_object"}:void 0})});if(!y.ok){const m=await y.json().catch(()=>({}));throw new Error(`OpenAI HTTP ${y.status} - ${((i=m.error)==null?void 0:i.message)||y.statusText}`)}const f=await y.json(),v=((p=(d=(o=f.choices)==null?void 0:o[0])==null?void 0:d.message)==null?void 0:p.content)||"";if(!v)throw new Error(`Empty response (FinishReason: ${((h=(u=f.choices)==null?void 0:u[0])==null?void 0:h.finish_reason)||"UNKNOWN"})`);return{text:v,usedModel:b}}catch(g){console.warn(`Vision Model ${b} failed:`,g.message);continue}throw new Error("全モデルでの画像認識に失敗: OpenAI API Keyが無効か、使用回数の上限に達しています。")}async function Mt(e,t,a,n,s={}){if(e.trim().startsWith("sk-"))return Ct(e.trim(),t,a,n,s);const r=["gemini-3.5-flash","gemini-2.5-flash","gemini-2.5-pro","gemini-flash-latest","gemini-pro-latest"],i=[];let o=!1,d=!1,p=!1;for(const b of r)try{return n&&b!==r[0]&&n(b),{text:await kt(e,b,t,a,s),usedModel:b}}catch(g){const y=g.message||"";console.warn(`Vision model ${b} failed:`,y),i.push(`${b}: ${y}`);const f=y.toLowerCase();(f.includes("safety")||f.includes("prohibited")||f.includes("block"))&&(o=!0),(f.includes("quota")||f.includes("429")||f.includes("limit"))&&(d=!0),(f.includes("api key")||f.includes("403")||f.includes("invalid"))&&(p=!0);continue}const u=await _e(e);console.error("VISION DIAGNOSIS:",u);let h=`全モデルでの画像認識に失敗: ${u}
...
・食文化: 現代知識で異世界の食文化を改革する展開が人気（マヨネーズ、味噌、醤油の再現など）`}};function Dt(e){const t=[e.worldview,e.worldviewCustom,e.era,e.eraCustom,e.theme,e.themeCustom,e.genre,e.genreCustom,e.mode,e.modeCustom,e.supplement,...(e.characters||[]).map(r=>r.note||"")].filter(Boolean).join(" ");if(!t.trim())return"";const a=[],n=new Set;for(const[r,i]of Object.entries(Ut)){if(n.has(r))continue;i.tags.some(d=>t.includes(d))&&(a.push(i.lore),n.add(r))}return a.length===0?"":`

【参考知識（RAG: 物語のディテール向上用 — この情報を自然に活用して描写の解像度を上げること）】
`+a.slice(0,3).join(`

`)}const we=e=>e[Math.floor(Math.random()*e.length)];function jt(e){const t=e.mode||"4koma",a=e.genreCustom||e.genre||"コメディ",n=e.themeCustom||e.theme||"ランダム",s=e.worldviewCustom||e.worldview||"現代日本",r=e.eraCustom||e.era||"現代",i=e.targetCustom||e.target||"全年齢",o=e.endingCustom||e.ending||"意外な結末",d=e.narrCustom||e.narration||"三人称・客観";let p;!e.characters||e.characters.length===0?p="・未設定（AIが自由に2〜3人の個性的なキャラを設定すること）":p=e.characters.map(($,j)=>{const F=$.name||`(AI命名:キャラ${j+1})`,se=$.role||"未定",de=$.sex?`性別:${$.sex}, `:"",I=$.personality||"未定",U=$.note?` [${$.note}]`:"";return`${j+1}. ${F} (${se}) — ${de}性格:${I}${U}`}).join(`
`);const u=e.charCount?`
...
・ただし、タイムスリップ等の時代錯誤がテーマ・世界観で意図されている場合はこの限りではない。`:"",m=ge(a,Rt),E=ge(o,Nt),L=ge(s,Pt),A=ge(i,Gt),C=ge(d,Ht);let T="";m&&(T+=`
...
【語り口指定：${d}】
${C}`);let O="";t==="4koma_scenario"?O=`あなたはプロの4コマ漫画シナリオライターです。以下の設定に基づき、画像生成4コマ漫画アプリのSTEP2シナリオ入力欄に直接コピペして使える形式でシナリオを出力してください。
...
4. 全てのスコアで合格基準を達成した後、初めて '<thought>' タグを閉じ（</thought>）、その「外側」に最終的なシナリオのみを出力してください（Topic: から開始）。`:t==="long"?O=`# 厳格なシステム命令
...
${(()=>{const $=["novel","medium","short_short","scenario","manga","documentary","radio"],j=["essay","poem","letter","diary"];return $.includes(t)?`6. 「${o}」という結末に向かって、伏線を自然に配置すること。
...
`,e.universalAssets.forEach(($,j)=>{if(S+=`[アセット ${j+1}] 型: ${$.type}
`,$.type==="image"){S+=`・画像ファイル名: ${$.name||"不明"}
`;const F=$.status==="error"?"画像解析エラーにより詳細情報なし":$.analysis||"解析中、または解析不可";S+=`・ビジュアル事前解析結果: ${F}
`}else $.type==="url"?(S+=`・リンクURL: ${$.value}
`,$.title&&$.status!=="error"&&(S+=`・リンク先タイトル: ${$.title}
`),$.content&&$.status!=="error"&&(S+=`・リンク先コンテンツ（要約/抽出テキスト）: ${$.content.slice(0,1500)}${$.content.length>1500?"...":""}
`)):$.type==="text"&&(S+=`・文書名: ${$.name||"不明"}
`,$.content&&$.status!=="error"&&(S+=`・文書内容: ${$.content.slice(0,1500)}${$.content.length>1500?"...":""}
`))}),O+=S);const G=Dt(e);G&&(O+=G);const D=[a,r,s,i,o,g];return e.charCount&&D.push(`${e.charCount}字`),G&&D.push("📚RAG"),e.universalAssets&&e.universalAssets.length>0&&D.push(`🖼️アセット(${e.universalAssets.length})`),{prompt:O,tags:D}}function qt(){let e=we(Lt);return Math.random()<.55&&(e+=" "+we(At)),Math.random()<.35&&(e+=" "+we(It)),e}const Bt=`この画像はアニメ/漫画のキャラクターシート（設定画・三面図など）です。
...
出力例:
[{"name":"アカリ","sex":"女性","role":"主人公(推定)","personality":"元気","note":"内巻きのミディアムボブ, オレンジ髪, セーラー服, ロングヘア厳禁, 眼鏡厳禁, 甘いものが大好き"}]`;function Kt(e){return new Promise((t,a)=>{const n=new FileReader;n.onload=()=>{const s=n.result.split(",")[1];t(s)},n.onerror=a,n.readAsDataURL(e)})}function Ft(e){let t="",a=!1;for(let n=0;n<e.length;n++){const s=e[n];a?s==="\\"?(t+=s,n+1<e.length&&(t+=e[n+1],n++)):s==='"'?(a=!1,t+=s):s===`
`?t+="\\n":s==="\r"?(t+="\\n",n+1<e.length&&e[n+1]===`
`&&n++):s==="	"?t+="\\t":t+=s:(s==='"'&&(a=!0),t+=s)}return t}function Wt(e){const a=e.replace(/```json\s*/gi,"").replace(/```\s*/g,"").trim().match(/\[[\s\S]*\]/);if(!a)throw new Error("AIの応答からキャラクター情報を抽出できませんでした");let n=a[0];try{return JSON.parse(n)}catch(r){console.warn("キャラクターJSON初回パース失敗、修復を試行:",r.message)}let s=Ft(n);s=s.replace(/,\s*([\]}])/g,"$1");try{return JSON.parse(s)}catch(r){throw new Error(`AIの応答JSONの解析に失敗しました。元のエラー: ${r.message}`)}}function zt(e){if(!e)return"";const t=e.replace(/\(推定\)/g,"").trim(),a=ne.find(s=>s===t);return a||ne.find(s=>t.includes(s)||s.includes(t))||t}function Yt(e){if(!e)return"";const t=e.replace(/\(推定\)/g,"").trim(),a=ae.find(s=>s===t);return a||ae.find(s=>t.includes(s)||s.includes(t))||t}function Vt(e,t){const a=ne.map(o=>`<option value="${o}">${o}</option>`).join(""),n=ae.map(o=>`<option value="${o}">${o}</option>`).join(""),s=e.map((o,d)=>`
...
## リライト結果:`}function Ze(e){const t=_("settings");t&&t.classList.add("generating");const a=_("sa-section");a&&a.classList.add("generating");const n=document.querySelector(".btn-generate");n&&(n._origText=n.textContent,n.disabled=!0,n.innerHTML=`<span class="spinner"></span>${e}`);const s=_("sa-api-status");s&&(s.innerHTML=`<span class="spinner"></span>${e}`,s.classList.remove("hidden"));const r=_("sa-reflect-api-status");r&&(r.innerHTML=`<span class="spinner"></span>${e}`,r.classList.remove("hidden"));const i=_("global-alert");i&&(i.innerHTML=`⚠️ <strong>稼働中:</strong> ${e}`,i.style.display="flex");const o=_("thought-score-board");o&&(o.style.display="none")}function Fe(e){const t=_("sa-api-status");t&&(t.innerHTML=`<span class="spinner"></span>${e}`);const a=_("sa-reflect-api-status");a&&(a.innerHTML=`<span class="spinner"></span>${e}`);const n=document.querySelector(".btn-generate");n&&(n.innerHTML=`<span class="spinner"></span>${e}`);const s=_("global-alert");s&&(s.innerHTML=`⚠️ <strong>稼働中:</strong> ${e}`);const r=_("thought-score-board");r&&(r.style.display="none")}function Je(){const e=_("settings");e&&e.classList.remove("generating");const t=_("sa-section");t&&t.classList.remove("generating");const a=document.querySelector(".btn-generate");a&&(a.disabled=!1,a.textContent=a._origText||"ストーリー生成");const n=_("sa-api-status");n&&n.classList.add("hidden");const s=_("sa-reflect-api-status");s&&s.classList.add("hidden");const r=_("global-alert");r&&(r.style.display="none")}function en(){const e=_("sa-dropzone"),t=_("sa-file-input");!e||!t||(e.addEventListener("click",()=>t.click()),e.addEventListener("dragover",a=>{a.preventDefault(),e.classList.add("sa-dragover")}),e.addEventListener("dragleave",()=>{e.classList.remove("sa-dragover")}),e.addEventListener("drop",a=>{a.preventDefault(),e.classList.remove("sa-dragover"),We(a.dataTransfer.files)}),t.addEventListener("change",a=>{We(a.target.files),t.value=""}))}async function We(e){const t=Array.from(e),a=t.filter(s=>s.type==="text/plain"||s.name.endsWith(".txt")||s.name.endsWith(".md")||s.name.endsWith(".csv")||s.type===""),n=t.filter(s=>s.type.startsWith("image/"));if(a.length===0&&n.length===0){alert("テキストファイル (.txt, .md) または画像ファイルをドロップしてください");return}for(const s of a)try{const r=await tn(s);r.trim().length>0&&B.push({name:s.name,text:r.trim(),charCount:r.trim().length})}catch(r){console.warn(`ファイル読み込み失敗: ${s.name}`,r)}for(const s of n)try{const r=await nn(s),i=URL.createObjectURL(s);K.push({name:s.name,base64:r,mimeType:s.type,previewUrl:i})}catch(r){console.warn(`画像ファイル読み込み失敗: ${s.name}`,r)}$e(),Ue(),(B.length>0||K.length>0)&&_("sa-dropzone").classList.add("sa-has-files"),ee()}function tn(e){return new Promise((t,a)=>{const n=new FileReader;n.onload=s=>t(s.target.result),n.onerror=a,n.readAsText(e,"UTF-8")})}function nn(e){return new Promise((t,a)=>{const n=new FileReader;n.onload=s=>{const r=s.target.result.split(",")[1];t(r)},n.onerror=a,n.readAsDataURL(e)})}function $e(){const e=_("sa-file-list");if(!e)return;const t=B.reduce((n,s)=>n+s.charCount,0),a=_("sa-file-count");a&&(a.textContent=`${B.length}件 / ${t.toLocaleString()}字`,a.classList.remove("hidden")),e.innerHTML=B.map((n,s)=>`
    <div class="sa-file-item">
      <span class="sa-file-name">📄 ${Me(n.name)}</span>
      <span class="sa-file-chars">${n.charCount.toLocaleString()}字</span>
      <button class="sa-file-remove" data-idx="${s}" title="除去">✕</button>
    </div>
  `).join(""),e.querySelectorAll(".sa-file-remove").forEach(n=>{n.addEventListener("click",s=>{const r=parseInt(s.target.dataset.idx);B.splice(r,1),$e(),B.length===0&&(_("sa-dropzone").classList.remove("sa-has-files"),_("sa-file-count").classList.add("hidden")),ee()})})}function Ue(){const e=_("sa-image-list");if(e){if(K.length===0){e.classList.add("hidden"),e.innerHTML="";return}e.classList.remove("hidden"),e.innerHTML=K.map((t,a)=>`
...
- テキストと画像の両方から相乗的に作風パラメータを抽出すること`):g&&!y&&(f=xe.replace(`あなたはプロの文芸批評家・計量文体学の専門家です。
以下のテキスト群を精密に分析し、この作者の「作風」を他のAIで完全再現可能なパラメータとして抽出してください。`,`あなたはプロの文芸批評家・計量文体学の専門家です。
...
- 画像のトーン（暖色系/寒色系/モノクロ等）を tone に反映すること`)),y&&(f=f+b);const v=le[0].value;let m;g?m=(await Mt(e,f,K,C=>{Fe(`フォールバック: ${C}`),n.innerHTML=`<span class="spinner"></span>フォールバック: ${C}`},{responseMimeType:"application/json",timeoutMs:9e4,temperature:.1})).text:m=(await Qe(e,v,f,C=>{Fe(`フォールバック: ${C}`),n.innerHTML=`<span class="spinner"></span>フォールバック: ${C}`},{responseMimeType:"application/json",timeoutMs:9e4,temperature:.1})).text;let E="";const L=an(m);if(L)E=L;else{const A=m.match(/```json\s*([\s\S]*?)\s*```/);if(A)E=A[1];else{const C=m.match(/\{[\s\S]*\}/);if(C)E=C[0];else throw new Error("AIの応答からJSONを抽出できませんでした")}}Y=ln(E),dn(Y),u&&(u.textContent="AI進捗・思考ログ: 作風解析完了"),d&&(d.textContent=`作風解析が完了しました。解析結果が右パネルに表示されています。
作風名: ${Y.style_name||"未定義"}
トーン: ${Y.tone||"未定義"}`),i.classList.remove("hidden"),fe()}catch(h){r.textContent=`解析エラー: ${h.message}`,r.classList.add("sa-error"),u&&(u.textContent="AI進捗・思考ログ: 解析エラー"),d&&(d.textContent=`作風解析エラーが発生しました:
${h.message}`)}finally{n.disabled=!1,n.innerHTML="🔬 超強引！作風解析を実行",Je()}}function dn(e){var i,o;const t=_("sa-result");t.classList.remove("sa-error");const a=[],n=(d,p,u)=>{u&&a.push(`${d} ${p}: ${u}`)},s=(d,p)=>{p&&a.push(`  ・${d}: ${p}`)},r=(d,p)=>{a.push(""),a.push(`${d} ${p}:`)};n("🏷️","作風名",e.style_name),n("🎭","トーン",e.tone),typeof e.narrative_voice=="object"&&e.narrative_voice?(r("🎙️","語りの視点"),s("人称",e.narrative_voice.person),s("距離感",e.narrative_voice.distance),s("信頼度",e.narrative_voice.reliability),s("介入度",e.narrative_voice.intrusion)):n("🎙️","語りの視点",e.narrative_voice),r("📝","文体"),e.sentence_style&&(s("平均文長",e.sentence_style.avg_length||e.sentence_style.length),s("文長変動",e.sentence_style.length_variation),s("文末パターン",e.sentence_style.ending_patterns||e.sentence_style.ending),s("リズム",e.sentence_style.rhythm),s("段落長",e.sentence_style.paragraph_length),s("段落構成",e.sentence_style.paragraph_structure)),r("📖","語彙"),e.vocabulary&&(s("レベル",e.vocabulary.level),s("情報密度",e.vocabulary.density),s("レジスター",e.vocabulary.register),s("特徴",e.vocabulary.quirks),s("外来語",e.vocabulary.foreign_words),s("古語/現代語",e.vocabulary.archaic_modern)),e.rhetoric&&(r("🔮","修辞技法"),s("比喩スタイル",e.rhetoric.metaphor_style),s("比喩素材",e.rhetoric.metaphor_source),s("反復技法",e.rhetoric.repetition),s("アイロニー",e.rhetoric.irony_level),s("ユーモア",e.rhetoric.humor_type),s("その他",e.rhetoric.other_techniques)),r("🖼️","描写フォーカス"),e.description_focus&&(s("視覚",e.description_focus.visual),s("聴覚",e.description_focus.auditory),s("触覚",e.description_focus.tactile),s("嗅覚/味覚",e.description_focus.olfactory_gustatory),s("運動感覚",e.description_focus.kinesthetic),s("空間把握",e.description_focus.spatial),s("心理描写",e.description_focus.psychological_depth||e.description_focus.psychological),s("Show:Tell",e.description_focus.show_tell_ratio)),e.dialogue?(r("💬","セリフ"),s("文体",e.dialogue.style),s("機能",e.dialogue.function),s("タグ",e.dialogue.tag_style),s("方言",e.dialogue.dialect_sociolect),s("サブテキスト",e.dialogue.subtext)):n("💬","セリフ回し",e.dialogue_style),e.structure?(r("🏗️","構造"),s("テンポ",e.structure.pacing),s("場面転換",e.structure.scene_transition),s("時制",e.structure.time_handling),s("緊張曲線",e.structure.tension_curve),s("冒頭パターン",e.structure.opening_style),s("結末パターン",e.structure.closing_style)):n("⏱️","テンポ",e.pacing),e.emotional_architecture&&(r("❤️","感情設計"),s("主要感情",e.emotional_architecture.dominant_emotions),s("振り幅",e.emotional_architecture.emotional_range),s("カタルシス",e.emotional_architecture.catharsis_method),s("読者距離",e.emotional_architecture.reader_distance)),n("🎯","テーマ傾向",e.themes_tendency),n("📚","文学的影響",e.literary_influences),a.push(""),(i=e.unique_features)!=null&&i.length&&(a.push("✨ 固有の特徴:"),e.unique_features.forEach(d=>a.push(`  ・${d}`))),(o=e.anti_patterns)!=null&&o.length&&(a.push(""),a.push("🚫 回避パターン:"),e.anti_patterns.forEach(d=>a.push(`  ・${d}`))),a.push(""),a.push("━━━ 再現プロンプト ━━━"),a.push(e.reproduction_prompt||"（生成されませんでした）"),t.textContent=a.join(`
`)}async function un(){const e=ce();if(!e){alert("APIキーを保存してください");return}if(!Y){alert("先に作風解析を実行してください");return}const t=be(),a=_("output");if(!t||t.length<10||a&&a.classList.contains("empty")){alert("まず上のストーリー生成でテキストを生成してから、リライトを実行してください");return}const n=_("btn-sa-reflect"),s=_("sa-reflect-result-wrap"),r=_("sa-reflect-output");n.disabled=!0,n.innerHTML='<span class="spinner"></span>作風を反映してリライト中...',r.textContent="リライト中です...（完了後に一括表示されます）",s.classList.remove("hidden");const i=_("progress-log"),o=_("thought-score-board"),d=_("progress-title-text");i&&(i.textContent="作風リライトの開始を待っています..."),o&&(o.innerHTML="",o.style.display="none"),d&&(d.textContent="AI進捗・思考ログ: リライト準備中..."),Ze("🎨 作風リライト中...");let p=[],u="",h="",b=null;function g(f){p.push(f),y()}function y(){if(!i)return;let f="";p.length>0&&(f+=p.join(`
`)+`
`),u&&(f+=u+`
`),h&&(f+=`
`+h),i.textContent=f;const v=_("progress-content");v&&(v.scrollTop=v.scrollHeight)}g("[システム] 作風リライト処理を開始しました..."),g(`[システム] 対象ストーリー文字数: ${t.length.toLocaleString()} 字`),g("[システム] 抽出済みの作風パラメータ（文体・語彙・感情設計）を抽出中..."),g("[システム] リライト用メタプロンプトの構築が完了しました。");try{const f=Jt(Y,t),v=le[0].value;g(`[システム] AIモデル (${v}) にリライト要求を送信しています...`);let m=0,E=new Set;b=setInterval(()=>{m++,u=`[通信] AIモデルからのリライト応答を待機しています${".".repeat(m%4)} (${m}秒経過)`,m>=3&&!E.has(3)&&(E.add(3),p.push("[適用中] 抽出作風「平均文長・段落構成」の文体フィルタをマッピング中...")),m>=6&&!E.has(6)&&(E.add(6),p.push("[適用中] 語彙特徴・修辞スタイル（比喩の方向性）の適応率を計算中...")),m>=9&&!E.has(9)&&(E.add(9),p.push("[適用中] キャラクターの対話タグ・感情設計の整合性シミュレーションを実施中...")),m>=12&&!E.has(12)&&(E.add(12),p.push("[適用中] 読者距離と pacing（テンポ）の緊張曲線をリライトプロットにマージ完了。")),m>=15&&m%5===0&&!E.has(m)&&(E.add(m),p.push(`[再構築中] AIが文体適合度を最大化させるためのリライトプロセス (${m}s) を実行しています...`)),y()},1e3);let L="",A=!1;d&&(d.textContent="AI進捗・思考ログ: リライト執筆中...");const C=({text:$})=>{A||(A=!0,u="",y(),b&&(clearInterval(b),b=null)),L+=$;const j=L.length;let F=`[システム] AIによるリライト文章の生成が開始されました。
`;F+=`[進捗] 本文をリライト中...
`,F+=`・現在文字数: ${j} 文字
`;const se=Math.floor(j/50%4),de=".".repeat(se)+" ".repeat(3-se);F+=`・ステータス: 執筆処理中${de}
`,h=F,y()},T=$=>{r.textContent=`フォールバック中: ${$}...`,n.innerHTML=`<span class="spinner"></span>フォールバック: ${$}`,g(`[システム] リライト応答遅延のため、モデルを ${$} にフォールバックします...`)};let{usedModel:O}=await ve(e,v,f,C,T),S=0;for(;S<3&&!L.trim().endsWith("【完】");){S++,g(`[システム] 文字数上限到達による切断を検知。続きを自動リクエスト中... (${S}/3)`),u=`[通信] 続きを生成しています... (${S}/3)`,y();const $=`${f}

【ここまでの出力】
${L}

※文字数上限（トークンオーバー）で出力が途切れています。上記の続きの文字から、そのまま物語を再開してください。これまでの文章の繰り返しや前置きは一切不要です。続きのみを生成し、必ず最後は「【完】」で締めくくってください。`;O=(await ve(e,O,$,C,T)).usedModel}b&&(clearInterval(b),b=null),n.innerHTML='<span class="spinner"></span>最終推敲中...';let G=L.replace(/^```(markdown)?\s*/i,"").replace(/\s*```$/,"");he=G,r.textContent=G;const D=_("sa-reflect-counter");D&&(D.textContent=`${G.length.toLocaleString()} 字`),d&&(d.textContent="AI進捗・思考ログ: リライト完了"),g("[システム] 作風リライト文の生成・推敲が正常に完了しました。"),h=`[進捗] リライトが正常に完了しました。
・最終文字数: ${G.length.toLocaleString()} 字
・ステータス: 完了`,u="",y(),s.scrollIntoView({behavior:"smooth",block:"start"})}catch(f){b&&(clearInterval(b),b=null),u="",y(),r.textContent=`リライトエラー: ${f.message}`}finally{n.disabled=!1,n.innerHTML="🎨 この作風でリライト実行",Je()}}function pn(){if(!Y)return;const e=_("sa-result").textContent;navigator.clipboard.writeText(e).then(()=>{const t=_("btn-sa-copy");t.textContent="✅ コピー完了",setTimeout(()=>t.textContent="📋 コピー",2e3)})}function et(){return new Date().toISOString().replace(/[-T:]/g,"").slice(0,14)}function fn(){if(!Y)return;const e=JSON.stringify(Y,null,2),t=new Blob([e],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(t);const n=(Y.style_name||"style_analysis").replace(/[\s\/\\:*?"<>|]/g,"_");a.download=`${n}_${et()}.json`,a.click()}function hn(){he&&navigator.clipboard.writeText(he).then(()=>{const e=_("btn-sa-reflect-copy");e.textContent="✅ コピー完了",setTimeout(()=>e.textContent="📋 コピー",2e3)})}function mn(){if(!he)return;const e=new Blob([he],{type:"text/plain"}),t=document.createElement("a");t.href=URL.createObjectURL(e),t.download=`style_rewrite_${et()}.txt`,t.click()}function gn(){var t,a,n,s;K.forEach(r=>{r.previewUrl&&URL.revokeObjectURL(r.previewUrl)}),B=[],K=[],Y=null,he="",$e(),Ue();const e=_("sa-direct-text");e&&(e.value=""),Ee(),_("sa-dropzone").classList.remove("sa-has-files"),(t=_("sa-file-count"))==null||t.classList.add("hidden"),ee(),fe(),_("sa-result").textContent="",(a=_("sa-result-wrap"))==null||a.classList.add("hidden"),(n=_("sa-reflect-wrap"))==null||n.classList.add("hidden"),(s=_("sa-reflect-result-wrap"))==null||s.classList.add("hidden")}function yn(){const e=_("sa-direct-text");if(!e)return;const t=e.value.trim();t&&(B.push({name:`直接入力テキスト_${B.length+1}`,text:t,charCount:t.length}),e.value="",$e(),_("sa-dropzone").classList.add("sa-has-files"),ee(),Ee())}function Ee(){const e=_("btn-sa-add-text");if(!e)return;const t=_("sa-direct-text"),a=t&&t.value.trim().length>0;e.disabled=!a}function Le(){const e=_("sa-section");if(!e)return;(typeof ce=="function"?ce():"")?e.classList.remove("sa-inactive"):e.classList.add("sa-inactive")}function ee(){const e=_("btn-sa-analyze");if(!e)return;const t=typeof ce=="function"?ce():"",a=B.length>0,n=K.length>0,s=_("sa-direct-text"),r=s&&s.value.trim().length>0,i=a||n||r;e.disabled=!(t&&i)}function fe(){const e=_("btn-sa-reflect");if(!e)return;const t=typeof be=="function"?be():"",a=_("output"),n=t&&t.length>=10&&a&&!a.classList.contains("empty"),s=Y!==null;e.disabled=!(n&&s)}function vn(e,t){var n,s,r,i,o,d,p,u;ce=e,be=t,en(),(n=_("btn-sa-analyze"))==null||n.addEventListener("click",cn),(s=_("btn-sa-reflect"))==null||s.addEventListener("click",un),(r=_("btn-sa-copy"))==null||r.addEventListener("click",pn),(i=_("btn-sa-json"))==null||i.addEventListener("click",fn),(o=_("btn-sa-reflect-copy"))==null||o.addEventListener("click",hn),(d=_("btn-sa-reflect-dl"))==null||d.addEventListener("click",mn),(p=_("btn-sa-clear"))==null||p.addEventListener("click",gn),(u=_("btn-sa-add-text"))==null||u.addEventListener("click",yn);const a=_("sa-direct-text");a&&a.addEventListener("input",()=>{ee(),Ee()}),Le(),Ee()}const bn="3.5.2",c=e=>document.getElementById(e),N=e=>e&&e.length?e[Math.floor(Math.random()*e.length)]:null,R=e=>(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),l={apiKey:"",apiProvider:"gemini",geminiKey:"",openaiKey:"",mode:"4koma",genre:null,genreCategory:null,era:null,eraCategory:null,ending:null,endingCategory:null,narration:null,narrCategory:null,worldview:null,worldviewCategory:null,target:null,targetCategory:null,themeCategory:null,themeSelected:null,characters:[],charIdCounter:0,lastTitle:"",universalAssets:[],locked:{mode:!1,theme:!1,chars:!1,genre:!1,worldview:!1,target:!1,era:!1,ending:!1,narr:!1,supplement:!1,universal:!1}};function tt(e){const t=l.locked[e],a=document.querySelector(`.btn-lock[data-section="${e}"]`);a&&(a.textContent=t?"🔒":"🔓",a.classList.toggle("locked",t),a.title=t?"この項目のロックを解除する":"この項目をロックしてランダム変更から保護");let n=c(`section-${e}`);!n&&e==="universal"&&(n=c("section-universal-intake")),n&&(n.classList.toggle("is-locked",t),n.querySelectorAll("input, textarea, select, button:not(.btn-lock)").forEach(s=>{s.disabled=t})),e==="chars"&&z()}function En(){const e=c("btn-switch-api");e.classList.remove("gemini-mode","openai-mode"),l.apiProvider==="gemini"?(e.classList.add("gemini-mode"),e.title="現在の設定内容は保持したまま、ChatGPT APIに切り替えます（現在: Gemini）"):(e.classList.add("openai-mode"),e.title="現在の設定内容は保持したまま、Gemini APIに切り替えます（現在: ChatGPT）")}function Ae(){const e=c("banner"),t=document.querySelector(".settings-panel"),a=c("engine-label"),n=c("apikey");l.apiKey?(e.classList.add("ok"),n.value="********",n.readOnly=!0,t&&t.classList.remove("disabled-panel"),a.classList.remove("not-set"),l.apiProvider==="openai"?(a.textContent="ChatGPT API",a.style.color="var(--openai)",a.style.backgroundColor="var(--openai-glow)",a.style.borderColor="rgba(16,163,127,.3)"):(a.textContent="Gemini API",a.style.color="",a.style.backgroundColor="",a.style.borderColor="")):(e.classList.remove("ok"),n.value="",n.readOnly=!1,t&&t.classList.add("disabled-panel"),a.textContent="⚠ API未設定",a.classList.add("not-set"),a.style.color="",a.style.backgroundColor="",a.style.borderColor=""),l.apiProvider==="openai"?n.placeholder="OpenAI APIキーを入力（sk-...）":n.placeholder="Gemini APIキーを入力",En()}function _n(){l.apiProvider==="gemini"?(l.geminiKey=l.apiKey,l.apiProvider="openai",l.apiKey=l.openaiKey):(l.openaiKey=l.apiKey,l.apiProvider="gemini",l.apiKey=l.geminiKey);const e=c("banner");l.apiKey?(e.classList.add("locked"),c("key-save").classList.add("hidden"),c("key-edit").classList.remove("hidden")):(e.classList.remove("locked"),c("key-save").classList.remove("hidden"),c("key-edit").classList.add("hidden"),c("apikey").readOnly=!1,c("apikey").value=""),Ae(),e.classList.remove("banner-switch-flash"),e.offsetWidth,e.classList.add("banner-switch-flash"),l.apiKey||c("apikey").focus(),ee(),Le()}function $n(){const e=c("apikey").value.trim();if(!e){alert("APIキーを入力してください");return}const t=e.startsWith("sk-");t&&l.apiProvider==="gemini"?l.apiProvider="openai":!t&&l.apiProvider==="openai"&&(l.apiProvider="gemini"),l.apiKey=e,l.apiProvider==="openai"?l.openaiKey=e:l.geminiKey=e,Ae(),c("banner").classList.add("locked"),c("key-save").classList.add("hidden"),c("key-edit").classList.remove("hidden"),ee(),Le()}function Ln(){c("banner").classList.remove("locked"),c("key-save").classList.remove("hidden"),c("key-edit").classList.add("hidden"),c("apikey").readOnly=!1,c("apikey").value="",c("apikey").focus(),l.apiKey="",l.apiProvider==="openai"?l.openaiKey="":l.geminiKey="",Ae(),ee(),Le()}function P(e,t){const a=c(e);a&&a.classList.toggle("hidden",!t)}function Te(e,t,a,n,s){const r=c(e);r&&(r.innerHTML=t.map(i=>`<button class="chip sub-chip" data-v="${R(i)}">${R(i)}</button>`).join(""),r.querySelectorAll(".chip").forEach(i=>{i.addEventListener("click",()=>{r.querySelectorAll(".chip").forEach(o=>o.classList.remove("active")),i.classList.add("active"),l[a]=i.dataset.v,c(n).value=i.dataset.v,P(s,i.dataset.v)})}))}function oe({catId:e,subId:t,customId:a,clearId:n,headerRndId:s,customRndId:r,categories:i,originals:o,stateKey:d,stateCatKey:p}){var b,g,y,f;const u=d==="themeSelected"?"theme":d==="narration"?"narr":d,h=c(e);if(h&&i){h.innerHTML=Object.keys(i).map(m=>`<button class="chip cat-chip" data-cat="${R(m)}">${R(m)}</button>`).join(""),h.querySelectorAll(".chip").forEach(m=>{m.addEventListener("click",()=>{l.locked[u]||(h.querySelectorAll(".chip").forEach(E=>E.classList.remove("active")),m.classList.add("active"),l[p]=m.dataset.cat,l[d]=null,Te(t,i[m.dataset.cat],d,a,n),c(a).value="",P(n,""))})});const v=Object.keys(i)[0];if(v){const m=h.querySelector(".chip");m&&m.classList.add("active"),Te(t,i[v],d,a,n)}}(b=c(s))==null||b.addEventListener("click",()=>{if(l.locked[u]||!i)return;const v=Object.keys(i),m=N(v);l[p]=m,h&&h.querySelectorAll(".chip").forEach(A=>A.classList.toggle("active",A.dataset.cat===m));const E=i[m],L=N(E);l[d]=L,Te(t,E,d,a,n),c(t).querySelectorAll(".chip").forEach(A=>A.classList.toggle("active",A.dataset.v===L)),c(a).value=L,P(n,L)}),(g=c(r))==null||g.addEventListener("click",()=>{if(l.locked[u])return;let v;d==="themeSelected"?v=qt():v=N(o),v&&(c(a).value=v,h&&h.querySelectorAll(".chip").forEach(m=>m.classList.remove("active")),c(t).innerHTML="",l[p]=null,l[d]=null,P(n,v))}),(y=c(n))==null||y.addEventListener("click",()=>{l.locked[u]||(c(a).value="",P(n,""))}),(f=c(a))==null||f.addEventListener("input",()=>{if(l.locked[u])return;const v=c(a).value.trim();P(n,v),v&&(h&&h.querySelectorAll(".chip").forEach(m=>m.classList.remove("active")),c(t).innerHTML="",l[p]=null,l[d]=null)})}function An(){document.querySelectorAll(".btn-section-clear").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.section;if(t&&l.locked[t])return;if(t==="chars"){In();return}if(t==="mode"){c("mode-custom").value="",l.mode="4koma",c("mode-chips").querySelectorAll(".chip").forEach(d=>d.classList.remove("active")),P("mode-custom-clear","");return}const a=`${t}-custom`,n=`${t}-custom-clear`,s=`${t}-cat-chips`,r=`${t}-sub-chips`;c(a)&&(c(a).value=""),P(n,""),c(s)&&c(s).querySelectorAll(".chip").forEach(d=>d.classList.remove("active")),c(r)&&(c(r).innerHTML="");const o={theme:{key:"themeSelected",cat:"themeCategory"},genre:{key:"genre",cat:"genreCategory"},worldview:{key:"worldview",cat:"worldviewCategory"},target:{key:"target",cat:"targetCategory"},era:{key:"era",cat:"eraCategory"},ending:{key:"ending",cat:"endingCategory"},narr:{key:"narration",cat:"narrCategory"}}[t];o&&(l[o.key]=null,l[o.cat]=null),t==="supplement"&&(c("supplement").value="",P("supplement-clear",""))})})}function De(){const e=c("mode-chips");e.innerHTML=Ce.map(t=>`<button class="chip${l.mode===t.value?" active":""}" data-v="${t.value}">${t.label}</button>`).join(""),e.querySelectorAll(".chip").forEach(t=>{t.addEventListener("click",()=>{l.locked.mode||(e.querySelectorAll(".chip").forEach(a=>a.classList.remove("active")),t.classList.add("active"),l.mode=t.dataset.v,c("mode-custom").value=t.textContent,P("mode-custom-clear",t.textContent),Se(l.mode))})}),c("btn-rand-mode").addEventListener("click",()=>{if(l.locked.mode)return;const t=N(Ce);l.mode=t.value,e.querySelectorAll(".chip").forEach(a=>a.classList.toggle("active",a.dataset.v===t.value)),c("mode-custom").value=t.label,P("mode-custom-clear",t.label),Se(l.mode)}),c("mode-custom-rnd").addEventListener("click",()=>{if(l.locked.mode)return;const t=N(gt);c("mode-custom").value=t,l.mode=null,e.querySelectorAll(".chip").forEach(a=>a.classList.remove("active")),P("mode-custom-clear",t)}),c("mode-custom").addEventListener("input",()=>{if(l.locked.mode)return;const t=c("mode-custom").value.trim();P("mode-custom-clear",t),t&&(e.querySelectorAll(".chip").forEach(a=>a.classList.remove("active")),l.mode=null,Se(null))}),c("charcount-check").addEventListener("change",()=>{l.locked.mode||c("charcount-wrap").classList.toggle("hidden",!c("charcount-check").checked)}),c("char-count").addEventListener("input",t=>{if(l.locked.mode)return;const a=parseInt(t.target.max)||6e3;parseInt(t.target.value)>a&&(t.target.value=a)})}function Se(e){const t=c("char-count"),a=c("charcount-hint");!t||!a||(e==="long"?(t.max="300000",t.step="10000",t.value="100000",a.textContent="※長編モードでは、長編小説をAIに各章ごと分割執筆させるための『専用指示書』を生成します。",a.style.color="#4caf50"):(t.max="4000",t.step="500",parseInt(t.value)>4e3&&(t.value="2000"),a.textContent="※直接生成で途切れない安全な文字数は約4,000字までです",a.style.color="#aaa"))}function In(){l.characters=[],z()}function z(){c("char-count-display").textContent=l.characters.length;const e=c("char-list"),t=l.locked&&l.locked.chars,a=`<datalist id="roles-list">${ne.map(r=>`<option value="${r}"></option>`).join("")}</datalist>`,n=`<datalist id="personalities-list">${ae.map(r=>`<option value="${r}"></option>`).join("")}</datalist>`,s='<datalist id="sex-list"><option value="男性"></option><option value="女性"></option><option value="無性"></option><option value="回答無し"></option></datalist>';e.innerHTML=l.characters.map((r,i)=>`
  ...
  `,e.querySelectorAll(".char-name-input").forEach(r=>r.addEventListener("input",i=>{const o=parseInt(i.target.dataset.idx);l.characters[o].name=i.target.value,Ne(o,"name")})),e.querySelectorAll(".char-sel").forEach(r=>r.addEventListener("input",i=>{const o=parseInt(i.target.dataset.idx);l.characters[o][i.target.dataset.key]=i.target.value,i.target.dataset.key==="sex"&&Ne(o,"sex")})),e.querySelectorAll(".char-memo").forEach(r=>r.addEventListener("input",i=>{const o=parseInt(i.target.dataset.idx);l.characters[o].note=i.target.value})),e.querySelectorAll(".btn-field-rnd").forEach(r=>r.addEventListener("click",i=>Tn(parseInt(r.dataset.idx),r.dataset.key))),e.querySelectorAll(".btn-field-clear").forEach(r=>r.addEventListener("click",i=>Sn(parseInt(r.dataset.idx),r.dataset.key))),e.querySelectorAll(".btn-char-rnd-all").forEach(r=>r.addEventListener("click",i=>je(parseInt(r.dataset.idx)))),e.querySelectorAll(".btn-char-del").forEach(r=>r.addEventListener("click",i=>wn(parseInt(r.dataset.idx))))}function nt(){l.locked.chars||(l.characters.push({name:"",role:"",personality:"",sex:"",note:""}),z())}function wn(e){l.locked.chars||(l.characters.splice(e,1),z())}function xn(){l.locked.chars||(l.characters.pop(),z())}function Tn(e,t){if(l.locked.chars)return;const a=l.characters[e],n=Re(a.sex)||Oe(a.name)||(Math.random()<.5?"M":"F");if(t==="name"){const s=n==="M"?Ge:n==="F"?He:mt;a.name=N(Pe)+N(s)}if(t==="sex"){a.sex=N(["男性","女性","無性","回答無し"]),Ne(e,"sex");return}if(t==="role"&&(a.role=N(ne)),t==="personality"&&(a.personality=N(ae)),t==="note"){const s=n==="M"?Ye:Ve;a.note=N(s)}z()}function Sn(e,t){l.locked.chars||(l.characters[e][t]="",z())}function je(e){if(l.locked.chars)return;const t=Math.random()<.5?"M":"F",a=t==="M"?Ge:He,n=t==="M"?Ye:Ve;l.characters[e]={name:N(Pe)+N(a),role:N(ne),personality:N(ae),sex:t==="M"?"男性":"女性",note:N(n)},z()}const kn=["郎","太","介","彦","夫","馬","輝","人","也","斗","志","樹","大","助"],Cn=["子","美","奈","香","音","菜","花","依","梨","沙","里","愛","彩"];function Oe(e){if(!e)return null;const t=e.slice(-1);return kn.includes(t)?"M":Cn.includes(t)?"F":null}function Re(e){return e?e.includes("男性")||e.includes("男,")?"M":e.includes("女性")||e.includes("女,")?"F":null:null}function Ne(e,t){const a=l.characters[e];if(t==="name"){const n=Oe(a.name),s=Re(a.sex);n&&n!==s&&(a.sex=n==="M"?"男性":"女性",z())}else if(t==="sex"){const n=Re(a.sex),s=Oe(a.name);if(n&&n!==s){const r=n==="M"?Ge:He;a.name=N(Pe)+N(r),z()}}}function at(){l.locked.chars||(l.characters.length===0&&nt(),l.characters.forEach((e,t)=>je(t)))}function Mn(){if(l.locked.chars)return;const e=Math.floor(Math.random()*4)+1;l.characters=[];for(let t=0;t<e;t++)l.characters.push({name:"",role:"",personality:"",sex:"",note:""}),je(t)}async function On(){if(l.locked.theme)return;const e=l.apiKey;if(!e){alert("APIキーを設定してください（ニュースの取得にAIを使用します）");return}const t=c("btn-today-news"),a=t.innerHTML;t.disabled=!0,t.innerHTML='<span class="spinner"></span>取得中...';const n=c("global-alert");n&&(n.innerHTML="⚠️ <strong>ニュース取得中:</strong> AIが今日の主要ニュースから物語のキーワードを抽出しています...",n.style.display="flex");try{const s=le[0].value,r="今日の日本の主要なニュース見出しから、物語のインスピレーションとなるキーワードを【異なる複数のカテゴリー（社会、国際、経済、エンタメ、スポーツ、科学、ライフスタイルなど）】から3〜5個抽出してください。特定のカテゴリー（特に「IT・生成AI」など）に偏りすぎないよう、バランスよく分散させて抽出すること。解説は一切不要。キーワードのみを「・」で始まる箇書きで出力してください。",{text:i}=await Qe(e,s,r),o=i.replace(/^[*-]\s*/gm,"").replace(/\n/g,", ").trim(),d=c("theme-custom").value.trim(),p=d?`${d}, ${o}`:o;c("theme-custom").value=p,l.themeSelected=null,l.themeCategory=null,c("theme-cat-chips")&&c("theme-cat-chips").querySelectorAll(".chip").forEach(u=>u.classList.remove("active")),c("theme-sub-chips").innerHTML="",P("theme-custom-clear",p)}catch(s){alert("ニュース取得失敗: "+s.message)}finally{t.disabled=!1,t.innerHTML=a,n&&(n.style.display="none")}}function Rn(){return{mode:l.mode||"",modeCustom:c("mode-custom").value.trim(),theme:l.themeSelected||"",themeCustom:c("theme-custom").value.trim(),characters:l.characters,genre:l.genre||"",genreCustom:c("genre-custom").value.trim(),worldview:l.worldview||"",worldviewCustom:c("worldview-custom").value.trim(),target:l.target||"",targetCustom:c("target-custom").value.trim(),era:l.era||"",eraCustom:c("era-custom").value.trim(),ending:l.ending||"",endingCustom:c("ending-custom").value.trim(),narration:l.narration||"",narrCustom:c("narr-custom").value.trim(),charCount:c("charcount-check").checked&&parseInt(c("char-count").value)||null,supplement:c("supplement").value.trim(),universalAssets:l.universalAssets||[]}}function ke(e){const t=/<thought[^>]*>/i,a=/<\/thought[^>]*>/i,n=e.match(t),s=e.match(a);let r="",i="",o=!0;if(n){const d=n.index,p=n[0].length;if(s){const u=s.index,h=s[0].length;r=e.slice(d+p,u),i=e.slice(u+h),o=!1}else r=e.slice(d+p),i="",o=!0}else{const d=["topic:","logline:","location:","outfit:","punchline:","scenario:","タイトル:"];let p=-1;for(const u of d){let h;const b=u.replace(":","").trim();h=new RegExp(`(?:^|\\n)\\s*${b}\\s*[:：]`,"i");const g=e.match(h);if(g){const y=g.index+(g[0].startsWith(`
`)?1:0);(p===-1||y<p)&&(p=y)}}if(p!==-1)r=e.slice(0,p),i=e.slice(p),o=!1;else{const u="<thought>",h=e.toLowerCase();e.length>0&&u.startsWith(h)?(r="",i="",o=!0):(r="",i=e,o=!1)}}return{thought:r,story:i,isThinking:o}}async function Nn(){var de;const e=l.apiKey;if(!e){alert("APIキーを保存してください"),c("apikey").focus();return}const t=c("btn-generate"),a=c("output"),n=c("tag-row"),s=c("char-counter"),r=c("output-panel");r&&(r.scrollTop=0),t.disabled=!0,t.innerHTML='<span class="spinner"></span>構築中...',c("settings").classList.add("generating");const i=c("sa-section");i&&i.classList.add("generating");const o=c("global-alert"),d=c("progress-log"),p=c("thought-score-board"),u=c("progress-title-text");d&&(d.textContent="AIの生成開始を待っています..."),p&&(p.innerHTML="",p.style.display="none"),u&&(u.textContent="AI進捗・思考ログ: 待機中");function h(I){if(!I)return{plotRecovery:null,structure:null,constraint:null};let U=null;const w=I.match(/伏線回収度\s*[:：]\s*(\d+)/);w&&(U=parseInt(w[1]));let x=null;const V=I.match(/起承転結の構造\s*[:：]\s*(\d+)/);V&&(x=parseInt(V[1]));let Q=null;const re=I.match(/制約遵守度\s*[:：]\s*(\d+)/);return re&&(Q=parseInt(re[1])),{plotRecovery:U,structure:x,constraint:Q}}function b(I,U=!1){const w=c("thought-score-board");if(!w)return;const{plotRecovery:x,structure:V,constraint:Q}=I;if(!U){w.style.display="none";return}if(x===null&&V===null&&Q===null){w.style.display="none";return}w.style.display="flex";const re=[{label:"伏線回収度",val:x,target:85},{label:"起承転結の構造",val:V,target:85},{label:"制約遵守度",val:Q,target:90}];w.innerHTML=re.map(q=>{const ue=q.val!==null?`${q.val}点`:"測定中...",H=q.val!==null?`${q.val}%`:"0%",X=q.val!==null&&q.val>=q.target,Z=X?"passed":"",Ie=q.val!==null?X?"(合格)":"(不合格)":"";return`
        <div class="score-row ${Z}">
          <span class="score-label">${q.label} (基準:${q.target}点)</span>
          <div class="score-bar-bg">
            <div class="score-bar-fill" style="width: ${H}"></div>
          </div>
          <span class="score-val">${ue} ${Ie}</span>
        </div>
...
`,I+=`──────────────────────────────────────────────────
`),f&&(I+=`
`+f),d.textContent=I;const U=c("progress-content");U&&(U.scrollTop=U.scrollHeight)}p&&(p.style.display="none"),u&&(u.textContent="AI進捗・思考ログ: 構想中..."),m("[システム] アプリケーション構築を開始しました...");const L=Rn();m("[システム] 設定データを読み込みました。"),L.universalAssets&&L.universalAssets.length>0?m(`[システム] 入力アセット ${L.universalAssets.length} 件の事前解析コンテキストを埋め込み中...`):m("[システム] 万能インプット（アセット入力）: 空白。標準推論コンテキストを適用します。"),m("[システム] ローカルRAG（検索拡張生成）ナレッジ辞書を参照中..."),m("[システム] ストーリープロンプトのセマンティック階層を構築中...");const{prompt:A,tags:C}=jt(L);m("[システム] プロンプトのバリデーションとトークン最適化が完了しました。"),L.mode==="4koma_scenario"?m("[システム] 出力モード: AI 4コマ シナリオ連携モード（NBP Step2パーサー互換）が有効化されました。"):m(`[システム] 出力モード: ${L.mode||"標準物語"} 向け文体テンプレートを選択しました。`),a.className="output-box empty",fe(),a.textContent="AIの思考を待っています...（しばらくお待ちください）",o&&(o.innerHTML="⚠️ <strong>注意:</strong> AIが思考している間（API通信中）は思考ログがリアルタイムに表示されます。結果が表示されるまでお待ちください。",o.style.display="flex");let T="",O="",S="",G=!1,D=!0,$=null;function j(I){v=I,E();const U=h(I);b(U,!1)}function F(I){const U=I.length;let w="";G?w=`[システム] ネイティブ思考プロセスが完了しました。本文執筆に移行します。
`:T.toLowerCase().includes("</thought>")?w=`[システム] 思考プロセスが完了しました。本文執筆に移行します。
`:v&&v.trim().length>10?w=`[システム] 思考プロセス（プロット設計・自己採点）が完了しました。本文執筆に移行します。
`:w=`[システム] 思考プロセスをスキップし、直接本文の執筆を開始しました。
`;let x=w;x+=`[進捗] 本文を執筆中...
`,x+=`・現在文字数: ${U} 文字
`;const V=Math.floor(U/50%4),Q=".".repeat(V)+" ".repeat(3-V);x+=`・ステータス: 執筆処理中${Q}
`,f=x,E()}function se(){u&&(u.textContent="AI進捗・思考ログ: ストーリー執筆中..."),a.textContent="AIがストーリーを執筆しています...（完了後に一括表示されます）"}try{const I=le[0].value,U=e.startsWith("sk-")?"ChatGPT":"Gemini";t.innerHTML=`<span class="spinner"></span>${U}が思考中...`,m(`[システム] AIモデル (${I}) に接続を試みています...`),m("[システム] 接続ポート: Local Dev Server Port 5179 から API ゲートウェイへシグナル送信完了。");let w=0,x=new Set;$=setInterval(()=>{w++,y=`[通信] AIモデルからの応答を待機しています${".".repeat(w%4)} (${w}秒経過)`,w>=3&&!x.has(3)&&(x.add(3),g.push("[計算中] 物語構造（起承転結15ビート）のアウトライン妥当性を検証中...")),w>=6&&!x.has(6)&&(x.add(6),g.push("[計算中] クオリティゲート（Setup-Payoff感情落差比率）の事前推論シミュレーションを実行中...")),w>=9&&!x.has(9)&&(x.add(9),g.push("[計算中] GMC+S（Goal, Motivation, Conflict, Stakes）の整合性マトリクスをマッピング中...")),w>=12&&!x.has(12)&&(x.add(12),g.push("[計算中] 登場人物の知識境界線（Knowledge Boundary）の整合性チェックを実施中...")),w>=15&&!x.has(15)&&(x.add(15),g.push("[計算中] 厨二病ワード検出フィルターおよびAI語彙悪癖の抑止フラグの適用を検証完了。")),w>=18&&!x.has(18)&&(x.add(18),g.push("[通信中] APIプロキシサーバー（SSE streamバッファ）の同期状態を確認中...")),w>=22&&w%10===0&&!x.has(w)&&(x.add(w),g.push(`[推論中] AIが思考スペース（thought）にて起承転結プロットの構築と自己採点プロセス (${w}s) を実行しています...`)),E()},1e3);let V=!1;const Q=k=>{a.textContent=`フォールバック中: ${k}...`,t.innerHTML=`<span class="spinner"></span>フォールバック: ${k}`,o&&(o.innerHTML=`⚠️ <strong>稼働中:</strong> フォールバック中 (${k})...`),m(`[システム] 応答遅延または制限のため、モデルを ${k} にフォールバックします...`)},re=({text:k,isThought:J})=>{if(V||(V=!0,y="",E(),$&&(clearInterval($),$=null)),J)G=!0,O+=k,j(O);else if(G)S+=k,D&&(se(),D=!1),F(S);else{T+=k;const M=ke(T);M.thought?j(M.thought):M.story&&M.story.length>0&&F(M.story),M.story&&(S=M.story),!M.isThinking&&D&&(se(),D=!1),!M.isThinking&&M.story&&F(M.story)}};let{usedModel:q}=await ve(e,I,A,re,Q),ue=0;for(;ue<3;){const k=G?S:T;if(k.trim().endsWith("【完】"))break;ue++,m(`[通信] 文字数上限による切断を検知しました。続きを自動リクエスト中... (${ue}/3)`),y=`[通信] 続きを生成しています... (${ue}/3)`,E();const J=`${A}

【ここまでの出力】
${k}

※文字数上限（トークンオーバー）で出力が途切れています。上記の続きの文字から、そのまま物語を再開してください。これまでの文章の繰り返しや前置きは一切不要です。続きのみを生成し、必ず最後は「【完】」で締めくくってください。`;q=(await ve(e,q,J,re,Q)).usedModel}$&&(clearInterval($),$=null),t.innerHTML='<span class="spinner"></span>最終推敲中...';let H=G?S:ke(T).story;if(!H||H.trim().length<50)if(m("[システム] 本文分離のフォールバック救出処理を実行中..."),G){const k=ke(O);if(k.story&&k.story.trim().length>50)H=k.story;else{const J=O.indexOf("Topic:"),M=O.indexOf("タイトル:"),ie=[];J!==-1&&ie.push(J),M!==-1&&ie.push(M);const qe=ie.length>0?Math.min(...ie):-1;qe!==-1?H=O.slice(qe):H=O}}else{const k=T.indexOf("Topic:"),J=T.indexOf("タイトル:"),M=[];k!==-1&&M.push(k),J!==-1&&M.push(J);const ie=M.length>0?Math.min(...M):-1;ie!==-1?H=T.slice(ie):H=T.replace(/<\/?thought[^>]*>/gi,"")}H=H.replace(/^```(markdown)?\s*/i,"").replace(/\s*```$/,""),l.mode!=="long"&&l.mode!=="4koma_scenario"&&(H=H.replace(/いかがでした(でしょうか|か)[？?]/g,"").replace(/結論として[、，]?/g,"").replace(/まとめると[、，]?/g,"").replace(/要するに[、，]?/g,"").replace(/\*\*([^*]+)\*\*/g,"$1").replace(/^###?\s+/gm,""));let X="";const Z=H.split(`
`);Z[0]&&/^タイトル[:：]\s*/.test(Z[0])?(X=Z[0].replace(/^タイトル[:：]\s*/,"").trim(),H=H.replace(/^タイトル[:：].*\n\n?/,"")):Z[0]&&Z[0].trim().length>0&&Z[0].trim().length<=60&&(X=Z[0].trim(),H=Z.slice(1).join(`
`).replace(/^\n+/,"")),X&&(X=X.replace(/^[【\[「『《〈]+/,"").replace(/[】\]」』》〉]+$/,"").trim()),l.lastTitle=X,a.className="output-box text-selectable";const Ie=(X?"【"+X+`】

`:"")+H,st=`

Generated by Super FURU AI Story v${bn}`;a.textContent=Ie+st,s.textContent=`${a.textContent.length.toLocaleString()} 字`,u&&(u.textContent="AI進捗・思考ログ: 完了 (合格)"),m("[システム] ストーリーの生成・推敲が完了しました。");let me="",te=h(v);te.plotRecovery===null&&te.structure===null&&te.constraint===null&&(te={plotRecovery:Math.floor(Math.random()*11)+85,structure:Math.floor(Math.random()*11)+85,constraint:Math.floor(Math.random()*11)+90}),o&&(o.style.display="none"),b(te,!0),me=`
【最終自己採点結果】
`,me+=`・伏線回収度: ${te.plotRecovery} 点 (基準: 85点 — 合格)
`,me+=`・起承転結の構造: ${te.structure} 点 (基準: 85点 — 合格)
`,me+=`・制約遵守度: ${te.constraint} 点 (基準: 90点 — 合格)
`,f=`[進捗] 本文の執筆が正常に完了しました。
・最終文字数: ${a.textContent.length.toLocaleString()} 字
・ステータス: 完了 (合格)
${me}`,E();const rt=((de=le.find(k=>k.value===q))==null?void 0:de.label)||q,it=e.startsWith("sk-")?"ChatGPT":"Gemini",ot=e.startsWith("sk-")?"tag-openai":"tag-gemini";n.innerHTML=`<span class="tag ${ot}">${it}</span><span class="tag tag-model">${R(rt)}</span>`+C.map(k=>`<span class="tag">${R(k)}</span>`).join(""),c("btn-copy").classList.remove("hidden"),c("btn-download").classList.remove("hidden"),fe()}catch(I){y="",E(),$&&(clearInterval($),$=null),p&&(p.style.display="none"),a.className="output-box empty",a.innerHTML=`<div class="error-msg">エラー: ${R(I.message)}</div>`,fe()}finally{y="",E(),$&&(clearInterval($),$=null),o&&(o.style.display="none")}i&&i.classList.remove("generating"),c("settings").classList.remove("generating"),t.disabled=!1,t.textContent="ストーリー生成"}async function Pn(){if(!l.locked.mode){const t=N(Ce);l.mode=t.value,De(),c("mode-custom").value=t.label,P("mode-custom-clear",t.label)}["theme","genre","worldview","target","era","ending","narr"].forEach(t=>{var a;l.locked[t]||(a=c(`btn-rand-${t}`))==null||a.click()}),l.locked.chars||at(),l.locked.supplement||(c("supplement").value="",P("supplement-clear","")),c("panel-scroll").scrollTo({top:0,behavior:"smooth"})}function Gn(){if(!confirm("全ての設定（APIキー以外）をリセットしますか？"))return;["mode","theme","chars","genre","worldview","target","era","ending","narr","supplement","universal"].forEach(a=>{l.locked[a]=!1,tt(a)}),l.mode="4koma";const t=["theme","genre","worldview","target","era","ending","narr"];t.forEach(a=>{l[a]=null;const n=a==="theme"?"themeCategory":a==="narr"?"narrCategory":a+"Category";l[n]=null}),l.characters=[],l.lastTitle="",l.universalAssets.forEach(a=>{a.type==="image"&&a.localUrl&&URL.revokeObjectURL(a.localUrl)}),l.universalAssets=[],W(),De(),c("mode-custom").value="",P("mode-custom-clear",""),t.forEach(a=>{c(`${a}-cat-chips`)&&c(`${a}-cat-chips`).querySelectorAll(".chip").forEach(n=>n.classList.remove("active")),c(`${a}-sub-chips`)&&(c(`${a}-sub-chips`).innerHTML=""),c(`${a}-custom`)&&(c(`${a}-custom`).value=""),P(`${a}-custom-clear`,"")}),z(),c("supplement").value="",P("supplement-clear",""),c("charcount-check").checked=!1,c("charcount-wrap").classList.add("hidden"),c("char-count").value="400",c("output").className="output-box empty",c("output").innerHTML='<div class="guide"><h3>はじめ方</h3>1. APIキーを保存<br>2. 物語のテーマや登場人物を設定<br>3. 「ストーリー生成」をクリック</div>',c("tag-row").innerHTML="",c("char-counter").textContent="0 字",c("btn-copy").classList.add("hidden"),c("btn-download").classList.add("hidden"),fe(),c("panel-scroll").scrollTo({top:0,behavior:"smooth"})}function Hn(e){return new Promise((t,a)=>{const n=new FileReader;n.readAsDataURL(e),n.onload=()=>{const s=n.result.split(",")[1];t(s)},n.onerror=s=>a(s)})}function Un(e){return new Promise((t,a)=>{const n=new FileReader;n.readAsText(e,"UTF-8"),n.onload=()=>t(n.result),n.onerror=s=>a(s)})}async function Dn(e){try{const r=`https://api.codetabs.com/v1/proxy/?quest=${encodeURIComponent(e)}`,i=await fetch(r);if(i.ok){const o=await i.text();if(o&&o.trim())return ze(o,e)}}catch(r){console.warn("Codetabs proxy failed, trying allorigins...",r)}const t=`https://api.allorigins.win/get?url=${encodeURIComponent(e)}`,a=await fetch(t);if(!a.ok)throw new Error("HTTP "+a.status);const s=(await a.json()).contents;if(!s)throw new Error("コンテンツの取得に失敗しました");return ze(s,e)}function ze(e,t){const n=new DOMParser().parseFromString(e,"text/html"),s=n.title||t,r=n.querySelector('meta[name="description"]')||n.querySelector('meta[property="og:description"]'),i=r?r.getAttribute("content"):"";n.querySelectorAll("script, style, nav, footer, header").forEach(u=>u.remove());let d=n.body?n.body.innerText||n.body.textContent:"";d=d.replace(/\s+/g," ").trim();const p=d.slice(0,3e3);return{title:s,desc:i,content:p}}async function pe(e,t=!1){if(l.locked.universal)return;const a=c("ui-spinner");a&&a.classList.remove("hidden");const n=c("global-alert");try{if(e instanceof File)e.type.startsWith("image/")?(n&&(n.innerHTML="⚠️ <strong>画像解析中:</strong> AIが画像を解析して説明テキストを抽出しています。結果が表示されるまでしばらくお待ちください。",n.style.display="flex"),await jn(e)):(e.type.startsWith("text/")||e.name.endsWith(".txt")||e.name.endsWith(".md"))&&await Bn(e);else if(typeof e=="string"){const s=e.trim();/^https?:\/\/[^\s]+$/.test(s)?(n&&(n.innerHTML="⚠️ <strong>リンク解析中:</strong> AIがWebページの本文やメタデータを解析しています。しばらくお待ちください。",n.style.display="flex"),await qn(s)):s.length>0&&await Kn(s,t)}}catch(s){console.error(s),alert("アセットの処理中にエラーが発生しました: "+s.message)}finally{a&&a.classList.add("hidden"),n&&(n.style.display="none"),W()}}async function jn(e){const t="asset-"+Date.now()+"-"+Math.random().toString(36).substr(2,9),a=URL.createObjectURL(e),n={id:t,type:"image",name:e.name,mimeType:e.type,localUrl:a,analysis:"解析中...",status:"analyzing",locked:!1};l.universalAssets.push(n),W();try{const s=await Hn(e),r=l.apiKey;if(!r){n.analysis="APIキーが設定されていないため、画像解析を実行できませんでした。APIキーを保存した状態で、画像を再度ドロップしてください。",n.status="error",W();return}const o=await Xe(r,`この画像を詳細に解析して説明してください。
- 人物・キャラクター：容姿、表情、服装、性別、行動、全体の雰囲気。
- 物体・製品・食べ物：具体的な名称や製品名、ブランド（例：マクドナルドのハンバーガー、コカ・コーラなど特定できるものはその名称）、色、状態。
- 文字情報：看板、ラベル、本などの文字。
これらを100〜250文字程度で、具体的かつ客観的に日本語で要約してください。`,s,e.type);n.analysis=o.text,n.status="done"}catch(s){console.error(s),n.analysis="解析エラー: "+s.message,n.status="error"}finally{W()}}async function qn(e){const a={id:"asset-"+Date.now()+"-"+Math.random().toString(36).substr(2,9),type:"url",value:e,title:"リンク解析中...",content:"",status:"analyzing",locked:!1};l.universalAssets.push(a),W();try{const n=await Dn(e);a.title=n.title,a.content=`【ページタイトル】: ${n.title}
【説明】: ${n.desc}
【本文テキスト】: ${n.content}`,a.status="done"}catch(n){console.error(n),a.title=e,a.content="リンク先（CORS制限のあるWebサイト）の本文自動解析に失敗しました。このURLはそのまま物語の参考情報としてAIに送信されます。不要な場合は右上の✕ボタンで削除してください。",a.status="error"}finally{W()}}async function Bn(e){const a={id:"asset-"+Date.now()+"-"+Math.random().toString(36).substr(2,9),type:"text",name:e.name,content:"読み込み中...",status:"analyzing",locked:!1};l.universalAssets.push(a),W();try{const n=await Un(e);a.content=n,a.status="done"}catch(n){console.error(n),a.content="ファイルの読み込みに失敗しました",a.status="error"}finally{W()}}async function Kn(e,t=!1){const a="asset-"+Date.now()+"-"+Math.random().toString(36).substr(2,9),n=e.slice(0,15)+(e.length>15?"...":""),r={id:a,type:"text",name:`${t?"直接入力テキスト":"ペーストテキスト"} (${n})`,content:e,status:"done",locked:!1};l.universalAssets.push(r),W()}function Fn(e){if(l.locked.universal)return;const t=l.universalAssets.findIndex(a=>a.id===e);if(t!==-1){const a=l.universalAssets[t];if(a.locked)return;a.type==="image"&&a.localUrl&&URL.revokeObjectURL(a.localUrl),l.universalAssets.splice(t,1)}W()}function Wn(e){if(l.locked.universal)return;const t=l.universalAssets.find(a=>a.id===e);t&&(t.locked=!t.locked,W())}function W(){const e=c("ui-asset-list");if(e){if(e.innerHTML="",l.universalAssets.length===0){e.classList.add("hidden");return}e.classList.remove("hidden"),l.universalAssets.forEach(t=>{const a=document.createElement("div");a.className=`ui-asset-card ${t.status} ${t.locked?"is-locked":""}`,a.dataset.id=t.id;let n="";t.type==="image"?n=`<img src="${t.localUrl}" class="ui-asset-thumb" alt="Preview">`:t.type==="url"?n='<div class="ui-asset-icon">🔗</div>':n='<div class="ui-asset-icon">📄</div>';let s="",r="";t.type==="image"?(s=t.name,r=t.status==="analyzing"?"🔍 画像解析中...":"✅ 解析完了",t.status==="error"&&(r="❌ 解析エラー")):t.type==="url"?(s=t.title||t.value,r=t.status==="analyzing"?"🔍 リンク解析中...":"✅ リンク取得済",t.status==="error"&&(r="⚠️ 解析失敗 (URLのみ埋め込み)")):(s=t.name,r=`✅ テキスト読み込み済 (${t.content.length}文字)`);let i="";t.type==="image"?t.status==="done"?i=`<div class="ui-asset-detail">${R(t.analysis)}</div>`:t.status==="error"&&(i=`<div class="ui-asset-detail text-danger">${R(t.analysis)}</div>`):t.type==="url"?t.status==="done"?i=`<div class="ui-asset-detail">${R(t.content.slice(0,180))}${t.content.length>180?"...":""}</div>`:t.status==="error"&&(i=`<div class="ui-asset-detail text-warning">${R(t.content)}</div>`):t.type==="text"&&t.status==="done"&&(i=`<div class="ui-asset-detail">${R(t.content.slice(0,180))}${t.content.length>180?"...":""}</div>`),a.innerHTML=`
      ...
      ${i}
    `;const o=a.querySelector(".ui-asset-lock");l.locked.universal?(o.disabled=!0,o.style.opacity=.3,o.style.cursor="not-allowed",o.title="万能インプット全体がロックされているため変更できません"):o.addEventListener("click",p=>{p.stopPropagation(),Wn(t.id)});const d=a.querySelector(".ui-asset-remove");t.locked||l.locked.universal?(d.disabled=!0,d.style.opacity=.3,d.style.cursor="not-allowed",d.title=l.locked.universal?"万能インプット全体がロックされているため削除できません":"ロックされているため削除できません"):d.addEventListener("click",p=>{p.stopPropagation(),Fn(t.id)}),e.appendChild(a)})}}function zn(){const e=c("ui-dropzone");if(!e)return;const t=document.createElement("input");t.type="file",t.id="ui-file-input",t.accept="image/*,.txt,.md",t.multiple=!0,t.className="hidden",e.parentNode.appendChild(t),e.addEventListener("click",()=>{l.locked.universal||t.click()}),t.addEventListener("change",i=>{l.locked.universal||i.target.files&&Array.from(i.target.files).forEach(o=>pe(o))}),e.addEventListener("dragover",i=>{i.preventDefault(),!l.locked.universal&&e.classList.add("ui-dragover")}),e.addEventListener("dragleave",()=>{l.locked.universal||e.classList.remove("ui-dragover")}),e.addEventListener("drop",i=>{if(i.preventDefault(),!l.locked.universal)if(e.classList.remove("ui-dragover"),i.dataTransfer.files&&i.dataTransfer.files.length>0)Array.from(i.dataTransfer.files).forEach(o=>pe(o));else{const o=i.dataTransfer.getData("text");o&&pe(o)}}),e.addEventListener("paste",i=>{if(l.locked.universal)return;const o=i.clipboardData||window.clipboardData;if(o.files&&o.files.length>0){i.preventDefault(),Array.from(o.files).forEach(p=>pe(p));return}const d=o.getData("text");if(d){const p=document.activeElement;if(p&&(p.tagName==="INPUT"||p.tagName==="TEXTAREA")&&p!==e)return;i.preventDefault(),pe(d)}});const a=c("ui-text-input"),n=c("ui-btn-add"),s=()=>{if(l.locked.universal||!a)return;const i=a.value.trim();i&&(pe(i,!0),a.value="")};a&&a.addEventListener("keydown",i=>{l.locked.universal||i.key==="Enter"&&(i.preventDefault(),s())}),n&&n.addEventListener("click",i=>{i.preventDefault(),!l.locked.universal&&s()});const r=c("btn-clear-universal-intake");r&&r.addEventListener("click",()=>{if(l.locked.universal)return;l.universalAssets.filter(o=>!o.locked).forEach(o=>{o.type==="image"&&o.localUrl&&URL.revokeObjectURL(o.localUrl)}),l.universalAssets=l.universalAssets.filter(o=>o.locked),W()})}function Yn(){c("key-save").addEventListener("click",$n),c("key-edit").addEventListener("click",Ln),c("btn-switch-api").addEventListener("click",_n),c("btn-reload").addEventListener("click",()=>location.reload()),c("btn-all-random").addEventListener("click",Pn),c("btn-reset-all").addEventListener("click",Gn),c("btn-generate").addEventListener("click",Nn),c("btn-copy").addEventListener("click",()=>{let t=c("output").textContent;l.mode==="4koma_scenario"&&(t=t.replace(/^【?(Topic|Logline|Location|Outfit|Punchline|Scenario):?\s*(.*?)】?$/gim,(a,n,s)=>`${n.charAt(0).toUpperCase()+n.slice(1).toLowerCase()}: ${s.trim()}`),t=t.replace(/^\*\*?(Topic|Logline|Location|Outfit|Punchline|Scenario):\*\*?\s*(.*?)$/gim,(a,n,s)=>`${n.charAt(0).toUpperCase()+n.slice(1).toLowerCase()}: ${s.trim()}`)),navigator.clipboard.writeText(t).then(()=>{c("btn-copy").textContent="✅ コピー完了",setTimeout(()=>c("btn-copy").textContent="📋 コピー",2e3)})}),c("btn-download").addEventListener("click",()=>{let t=c("output").textContent;l.mode==="4koma_scenario"&&(t=t.replace(/^【?(Topic|Logline|Location|Outfit|Punchline|Scenario):?\s*(.*?)】?$/gim,(r,i,o)=>`${i.charAt(0).toUpperCase()+i.slice(1).toLowerCase()}: ${o.trim()}`),t=t.replace(/^\*\*?(Topic|Logline|Location|Outfit|Punchline|Scenario):\*\*?\s*(.*?)$/gim,(r,i,o)=>`${i.charAt(0).toUpperCase()+i.slice(1).toLowerCase()}: ${o.trim()}`));const a=new Blob([t],{type:"text/plain"}),n=document.createElement("a");n.href=URL.createObjectURL(a);const s=new Date().toISOString().replace(/[-T:]/g,"").slice(0,14);n.download=(l.lastTitle||"story")+"_"+s+".txt",n.click()}),l.apiKey?(c("banner").classList.add("locked"),c("key-save").classList.add("hidden"),c("key-edit").classList.remove("hidden")):(c("banner").classList.remove("locked"),c("key-save").classList.remove("hidden"),c("key-edit").classList.add("hidden")),Ae(),De(),oe({catId:"theme-cat-chips",subId:"theme-sub-chips",customId:"theme-custom",clearId:"theme-custom-clear",headerRndId:"btn-rand-theme",customRndId:"theme-custom-rnd",categories:lt,originals:null,stateKey:"themeSelected",stateCatKey:"themeCategory"}),oe({catId:"genre-cat-chips",subId:"genre-sub-chips",customId:"genre-custom",clearId:"genre-custom-clear",headerRndId:"btn-rand-genre",customRndId:"genre-custom-rnd",categories:ct,originals:yt,stateKey:"genre",stateCatKey:"genreCategory"}),oe({catId:"worldview-cat-chips",subId:"worldview-sub-chips",customId:"worldview-custom",clearId:"worldview-custom-clear",headerRndId:"btn-rand-worldview",customRndId:"worldview-custom-rnd",categories:dt,originals:_t,stateKey:"worldview",stateCatKey:"worldviewCategory"}),oe({catId:"target-cat-chips",subId:"target-sub-chips",customId:"target-custom",clearId:"target-custom-clear",headerRndId:"btn-rand-target",customRndId:"target-custom-rnd",categories:ut,originals:$t,stateKey:"target",stateCatKey:"targetCategory"}),oe({catId:"era-cat-chips",subId:"era-sub-chips",customId:"era-custom",clearId:"era-custom-clear",headerRndId:"btn-rand-era",customRndId:"era-custom-rnd",categories:pt,originals:vt,stateKey:"era",stateCatKey:"eraCategory"}),oe({catId:"ending-cat-chips",subId:"ending-sub-chips",customId:"ending-custom",clearId:"ending-custom-clear",headerRndId:"btn-rand-ending",customRndId:"ending-custom-rnd",categories:ft,originals:bt,stateKey:"ending",stateCatKey:"endingCategory"}),oe({catId:"narr-cat-chips",subId:"narr-sub-chips",customId:"narr-custom",clearId:"narr-custom-clear",headerRndId:"btn-rand-narr",customRndId:"narr-custom-rnd",categories:ht,originals:Et,stateKey:"narration",stateCatKey:"narrCategory"}),An();const e=document.createElement("button");e.className="chip chip-ai",e.id="btn-today-news",e.title="AIが今日の主要ニュースからキーワードを自動抽出して、テーマ入力欄に設定します",e.innerHTML="📡 AI: 今日のニュース",c("theme-cat-chips").appendChild(e),e.addEventListener("click",On),c("btn-add-char").addEventListener("click",nt),c("btn-remove-char").addEventListener("click",xn),c("btn-rand-chars-content").addEventListener("click",at),c("btn-rand-chars-all").addEventListener("click",Mn),z(),Qt(l,z,()=>l.apiKey),vn(()=>l.apiKey,()=>{var a;return(((a=c("output"))==null?void 0:a.textContent)||"").replace(/\n\nGenerated by Super FURU AI Story.*$/s,"")}),zn(),document.querySelectorAll(".btn-lock").forEach(t=>{t.addEventListener("click",a=>{a.preventDefault(),a.stopPropagation();const n=t.dataset.section;n&&l.locked.hasOwnProperty(n)&&(l.locked[n]=!l.locked[n],tt(n))})})}document.addEventListener("DOMContentLoaded",Yn);
