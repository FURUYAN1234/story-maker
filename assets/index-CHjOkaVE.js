...
`)}const we=e=>e[Math.floor(Math.random()*e.length)];function jt(e){const t=e.mode||"4koma",a=e.genreCustom||e.genre||"コメディ",n=e.themeCustom||e.theme||"ランダム",s=e.worldviewCustom||e.worldview||"現代日本",r=e.eraCustom||e.era||"現代",i=e.targetCustom||e.target||"全年齢",o=e.endingCustom||e.ending||"意外な結末",d=e.narrCustom||e.narration||"三人称・客観";let p;!e.characters||e.characters.length===0?p="・未設定（AIが自由に2〜3人の個性的なキャラを設定すること）":p=e.characters.map(($,j)=>{const F=$.name||`(AI命名:キャラ${j+1})`,se=$.role||"未定",ce=$.sex?`性別:${$.sex}, `:"",I=$.personality||"未定",U=$.note?` [${$.note}]`:"";return`${j+1}. ${F} (${se}) — ${ce}性格:${I}${U}`}).join(`
`);const u=e.charCount?`
...
・ただし、タイムスリップ等の時代錯誤がテーマ・世界観で意図されている場合はこの限りではない。`:"",m=me(a,Rt),E=me(o,Nt),L=me(s,Pt),A=me(i,Gt),C=me(d,Ht);let T="";m&&(T+=`
