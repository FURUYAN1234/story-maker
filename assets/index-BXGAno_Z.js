...
・ただし、タイムスリップ等の時代錯誤がテーマ・世界観で意図されている場合はこの限りではない。`:"";let A="";e==="long"?A=`# 厳格なシステム命令
...
\`\`\`
`:A=`あなたはプロの書き手です。以下の詳細設定に基づき、読む人の心を動かす「${w}」を執筆してください。
...
7. 文体の緩急：物語全体を通じて同じトーン・テンポで書き続けないこと。主人公の心理状態の変化に応じて、文体自体にも変化を持たせること（例：焦燥時は短文の連続、回想時はゆったりとした長文、決断時は力強い断言調など）。特に物語の最大の衝撃・転換点（真相の判明、裏切りの発覚、喪失の瞬間など）では、「息を飲んだ」の一文で処理せず、主人公の思考の断片化・身体反応・時間感覚の歪みなどを通じて、読者にも衝撃を追体験させること。${d}${m}${u}${f}
    ...
    <div class="char-card shadow-sm">
      ...
      <div class="char-field-row">
        <div class="input-wrap">
          <input type="text" class="char-name-input" value="${L(s.name)}" data-idx="${r}" placeholder="例：山田太郎（空欄でAIお任せ）">
        </div>
        <div class="btn-group">
      ...
      <div class="char-field-row">
        <div class="input-wrap">
          <input type="text" class="char-sel" list="roles-list" data-idx="${r}" data-key="role" value="${L(s.role)}" placeholder="例：主人公、ライバル（空欄でAIお任せ）">
        </div>
        <div class="btn-group">
      ...
      <div class="char-field-row">
        <div class="input-wrap">
          <input type="text" class="char-sel" list="personalities-list" data-idx="${r}" data-key="personality" value="${L(s.personality)}" placeholder="例：熱血、クール（空欄でAIお任せ）">
        </div>
        <div class="btn-group">
      ...
      <div class="char-field-row">
        <div class="input-wrap">
          <textarea class="char-memo" data-idx="${r}" placeholder="例：男性, 短髪, 眼鏡（性別を変えると名前も自動調整）">${L(s.note)}</textarea>
        </div>
        <div class="btn-group">
